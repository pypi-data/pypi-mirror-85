...
class ElectrolyzerTechnicalEvaluation(TechnicalEvaluation):
    ...
    def power_auxilliaries_1_plotting(self):
        ...
        xaxis: Axis = Axis(data=Plotting.format_time(data.time), label=ElectrolyzerState.TIME)
        yaxis: [Axis] = list()
        yaxis.append(Axis(data.power_water_heating, label=ElectrolyzerState.POWER_WATER_HEATING, color=PlotlyPlotting.Color.BLUE))
        yaxis.append(Axis(data.power_pump, label=ElectrolyzerState.POWER_PUMP, color=PlotlyPlotting.Color.MAGENTA))
        plot.lines(xaxis, yaxis, [1])
        self.extend_figures(plot.get_figures())
    ...
    def power_auxilliaries_2_plotting(self):
        ...
        xaxis: Axis = Axis(data=Plotting.format_time(data.time), label=ElectrolyzerState.TIME)
        yaxis: [Axis] = list()
        yaxis.append(Axis(data.power_compressor, label=ElectrolyzerState.POWER_COMPRESSOR, color=PlotlyPlotting.Color.BLACK))
        yaxis.append(Axis(data.power_gas_drying, label=ElectrolyzerState.POWER_GAS_DRYING, color=PlotlyPlotting.Color.RED))
        plot.lines(xaxis, yaxis, [1])
        self.extend_figures(plot.get_figures())
    ...
    def degradation_plotting(self):
        ...
        xaxis: Axis = Axis(data=Plotting.format_time(data.time), label=ElectrolyzerState.TIME)
        yaxis: [Axis] = list()
        yaxis.append(Axis(data=data.resistance_increase, label=ElectrolyzerState.RESISTANCE_INCREASE, color=PlotlyPlotting.Color.RED))
        yaxis.append(Axis(data=data.resistance_increase_cyclic, label=ElectrolyzerState.RESISTANCE_INCREASE_CYCLIC, color=PlotlyPlotting.Color.BLUE))
        yaxis.append(Axis(data=data.resistance_increase_calendar, label=ElectrolyzerState.RESISTANCE_INCREASE_CALENDAR, color=PlotlyPlotting.Color.MAGENTA))
        yaxis.append(Axis(data=data.exchange_current_dens_decrease, label=ElectrolyzerState.EXCHANGE_CURRENT_DENS_DECREASE, color=PlotlyPlotting.Color.GREEN))
    ...
    def soh_plotting(self):
        ...
        xaxis: Axis = Axis(data=Plotting.format_time(data.time), label=ElectrolyzerState.TIME)
        yaxis: [Axis] = list()
        yaxis.append(Axis(data=data.soh, label=ElectrolyzerState.SOH))
