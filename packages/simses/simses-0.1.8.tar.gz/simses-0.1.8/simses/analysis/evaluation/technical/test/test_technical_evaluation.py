...
# Capacity_remaining:
@pytest.mark.parametrize('capacity, result',
                          [
                              ([10, 1], 10),
                              ([10, 10], 100),
                              ([10, 0], 0),
...
def test_capacity_remaining(capacity, result):

    system_dict = {SystemState.TIME: time_test, SystemState.CAPACITY: capacity,
                   SystemState.SYSTEM_AC_ID: 1, SystemState.SYSTEM_DC_ID: 1}
    system_data = SystemData(gen_sim_config, pandas.DataFrame(system_dict))
