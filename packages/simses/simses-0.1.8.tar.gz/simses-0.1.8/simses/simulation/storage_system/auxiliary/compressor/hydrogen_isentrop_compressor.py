...
class HydrogenIsentropCompressor(Compressor):
    ...
    def calculate_compression_power(self, hydrogen_flow_out:float, pressure_1: float, pressure_2: float, temperature: float) -> None:
        real_gas_faktor = ConstantsHydrogen.realgas_factor(pressure_1, pressure_2)
        isentropic_exponent = ConstantsHydrogen.isentropic_exponent(pressure_1, pressure_2)#
        prefactor = isentropic_exponent / (isentropic_exponent - 1)
        self.__compression_power = 1 / self.__compressor_eta * prefactor * self.__R * (temperature + 273.15) * \
