...
class FixCOPHeatingVentilationAirConditioningPIDControl(HeatingVentilationAirConditioning):
    ...
    def __init__(self, hvac_configuation: dict):
        super().__init__()
        ...
        try:
            self.__max_thermal_power: float = float(hvac_configuation['constant_hvac'][1])
        except IndexError:
            self.__max_thermal_power: float = 1000000

        self.__min_thermal_power = self.__max_thermal_power * 0

        try:
            self.__battery_temperature_target = float(hvac_configuation['constant_hvac'][2]) + 273.15
        except IndexError:
            self.__battery_temperature_target = 298.15

        try:
            self.__kp_coefficient = float(hvac_configuation['constant_hvac'][3])
        except IndexError:
            self.__kp_coefficient = 600

        try:
            self.__ki_coefficient = float(hvac_configuation['constant_hvac'][4])
        except IndexError:
            self.__ki_coefficient = 5

        try:
            self.__kd_coefficient = float(hvac_configuation['constant_hvac'][5])
        except IndexError:
