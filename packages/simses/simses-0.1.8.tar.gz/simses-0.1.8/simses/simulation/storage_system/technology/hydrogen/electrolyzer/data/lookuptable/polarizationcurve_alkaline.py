...
from simses.simulation.storage_system.technology.hydrogen.electrolyzer.data.parameters.parameters_alkaline_electrolyzer_fit import ParametersAlkalineElectrolyzerFit
...
lookup_table_activation_overpotential = pd.read_csv('activation_overpotential_lookup_alkaline.csv', delimiter=';', decimal=",")

fitting_parameters = ParametersAlkalineElectrolyzerFit(electrolyzer_data_config)
...
def calculate_theoretical_voltage(temperature: float, pressure: float) -> float:
    ...
    v_theoretical = v_reversible_standard_conditions + (
                (ConstantsHydrogen.IDEAL_GAS_CONST * temperature) / (2 * ConstantsHydrogen.FARADAY_CONST)) * np.log(
...
def calculate_activation_overvoltage_anode(current: float, temperature: float) -> float:
    ...
    tafel_slope = (ConstantsHydrogen.IDEAL_GAS_CONST * temperature) / (2*ConstantsHydrogen.FARADAY_CONST * alpha_a)
...
def calculate_activation_overvoltage_cathode(current: float, temperature: float) -> float:
    ...
    # Calculate tafel slope for anode and cathode
    tafel_slope = (ConstantsHydrogen.IDEAL_GAS_CONST * temperature) / (2*ConstantsHydrogen.FARADAY_CONST * alpha_c)
    # Calculate exchange current densities for anode and cathode for HRI Electrolyser (Ni Electrodes) TODO more general approach?
...
def display_example_curve(temperature: float, pressure: float, compareUI: bool):
    ...
    if compareUI:
        ...
        u_i_curve_path = 'UICurves\\StuartHRI\\'
        u_i_files = glob.glob(u_i_curve_path + '*.csv')
        for curve in u_i_files:
            excelfile = pd.read_csv(curve, delimiter=';', decimal=',')
            file_current_densities = excelfile.iloc[:, 0]
...
def cell_voltage_fit_model_function(x, ac, bc, cc, aa, ba, ca, r1, r2, r3, a1, a2):
    ...
    factor_anode = (ConstantsHydrogen.IDEAL_GAS_CONST * temperature) / (
                2 * ConstantsHydrogen.FARADAY_CONST * alpha_a)
    factor_cathode = (ConstantsHydrogen.IDEAL_GAS_CONST * temperature) / (
            2 * ConstantsHydrogen.FARADAY_CONST * alpha_c)
