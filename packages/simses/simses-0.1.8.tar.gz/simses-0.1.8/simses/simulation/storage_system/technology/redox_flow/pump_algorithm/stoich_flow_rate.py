...
class StoichFlowRate(PumpAlgorithm):
    ...
    def __init__(self, stack_module: StackModule, pump: Pump, electrolyte_system: ElectrolyteSystem,
        ...
        self.__current_max = self.__stack_module.get_absolute_max_current()
    ...
    def get_flow_rate(self, redox_flow_state: RedoxFlowState) -> float:
        ...
        if redox_flow_state.is_charge:
            delta_soc = 1 - redox_flow_state.soc_stack
        else:
            delta_soc = redox_flow_state.soc_stack
    ...
    def get_flow_rate_max(self) -> float:
        delta_soc_min = self.__redox_flow_config.min_soc
        flow_rate_max = (self.__stoichiometry * self.__current_max * self.__stack_module.get_cell_per_stack() *
                         self.__stack_module.get_serial_scale() / (self.__FARADAY *
                         self.__electrolyte_system.get_vanadium_concentration() * delta_soc_min))
