...
from simses.simulation.storage_system.technology.hydrogen.electrolyzer.stack_model.alkaline_electrolyzer_model.alkaline_pressure_model import \
    AlkalinePressureModel
from simses.simulation.storage_system.technology.hydrogen.electrolyzer.stack_model.alkaline_electrolyzer_model.alkaline_fluid_model import \
    AlkalineFluidModel
...
class AlkalineElectricalModel:
    ...
    def __init__(self, pressure_model: AlkalinePressureModel, fluid_model: AlkalineFluidModel, electrolyzer_data_config: ElectrolyzerDataConfig, parameters):
        ...
        self.__power_curve = pd.read_csv(self.__POWER_FILE, delimiter=';', decimal=",")
    ...
    def get_current(self, power_cell: float, state: ElectrolyzerState):
        pressure = state.total_pressure     # bar (pressure assumed to be identical for cathode and anode) TODO Check Validity
        stack_temperature = state.temperature     # °C
    ...
    def __theoretical_voltage(self, state: ElectrolyzerState, current) -> float:
        ...
        v_theoretical = v_reversible_standard_conditions + (
                (ConstantsHydrogen.IDEAL_GAS_CONST * stack_temperature) / (2 * ConstantsHydrogen.FARADAY_CONST)) * math.log(
    ...
    def __activation_voltage_anode(self, state: ElectrolyzerState, current: float) -> float:
        ...
        factor_anode = (ConstantsHydrogen.IDEAL_GAS_CONST * stack_temperature) / (
    ...
    def __activation_voltage_cathode(self, state: ElectrolyzerState, current: float) -> float:
        ...
        factor_cathode = (ConstantsHydrogen.IDEAL_GAS_CONST * stack_temperature) / (
