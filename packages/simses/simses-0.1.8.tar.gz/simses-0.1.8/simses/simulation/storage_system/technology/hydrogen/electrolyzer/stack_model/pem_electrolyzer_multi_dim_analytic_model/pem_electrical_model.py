...
from simses.simulation.storage_system.technology.hydrogen.electrolyzer.stack_model.pem_electrolyzer_multi_dim_analytic_model.pem_membrane_model import \
    PemMembraneModel
from simses.simulation.storage_system.technology.hydrogen.electrolyzer.stack_model.pem_electrolyzer_multi_dim_analytic_model.pem_pressure_model import \
    PemPressureModel
...
class PemElectricalModel:
    ...
    def __nernst_voltage_for_current_calc(self, state: ElectrolyzerState):
        ...
        return u_rev + self.__parameters.q7 * ConstantsHydrogen.IDEAL_GAS_CONST * (stack_temperature + 273.15) / \
               (2 * ConstantsHydrogen.FARADAY_CONST) * numpy.log((p_o2 / self.P_O2_REF) ** (1 / 2) * p_h2 / self.P_H2_REF)
