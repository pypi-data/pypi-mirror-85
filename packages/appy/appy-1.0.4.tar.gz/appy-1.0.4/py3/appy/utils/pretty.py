...
class PrettyPrinter:
    ...
    # Regular expression allowing to identify, within a Rich value, a "a" tag
    # containing a link to an external code file.
    codeLink = re.compile('<a href="(.*?)">py:\s*(.*?)\s*</a>')
    ...
    @classmethod
    def getFromXhtml(class_, xhtml, o=None):
        '''Replace any link found in this chunk of p_xhtml, pointing to an
           external code file, by the appropriate part of the code.'''
        return class_.codeLink.sub(
                 lambda m: class_.getFromUrl(*m.groups(), o=o), xhtml)
#- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
