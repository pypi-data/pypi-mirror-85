...
class BomWater():
    ...
    def check_cache_status(self):
        # current_dir = os.getcwd()
        # print(current_dir)
        if os.path.exists('.\cache\waterML_GetCapabilities.json'):

            return
        else:
            print(f'creating cache directory and files, this will take a little time please wait.')
            response = self.requests(self.actions.GetCapabilities)
            os.mkdir('./cache')
            self.xml_to_json_via_file(response.text, './cache/waterML_GetCapabilities.json')
            response = self.requests(self.actions.GetFeatureOfInterest)
            self.xml_to_json_via_file(response.text, './cache/station.json')
