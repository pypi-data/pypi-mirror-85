...
class SeismicStoreAPI(API):
    ...
    def search(
        ...
        *,
        ...
        """
        if get_all:
            req = SearchSeismicStoresRequest(include_file_info=include_file_info, include_headers=include_headers)
        else:
            spec = get_search_spec(id, external_id, external_id_substring, name, name_substring)
            req = SearchSeismicStoresRequest(
                seismic_stores=spec, include_file_info=include_file_info, include_headers=include_headers
            )
