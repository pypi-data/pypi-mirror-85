...
def write_config_file():
    ...
    if os.path.isfile(conf):
        LOGGER.info("\tFile already exists - updating it.")
        os.remove(conf)
...
exclude:
    ...
    LOGGER.info("... File: .reslate-config.yaml written successfully.")
...
def find_config_file():
    ...
    if not directory:
        LOGGER.error("Unable to find a .reslate-config.yaml file!")
        return None, None
...
def main():
    ...
    if args.initialise:
        LOGGER.info("Attempting to write .reslate-config.yaml file...")
        write_config_file()
    ...
    CONFIG.source = os.path.join(
        proj_root, os.path.normpath(config["package_source_path"])
    )
    CONFIG.docs = os.path.join(proj_root, os.path.normpath(config["docs_source_path"]))
    ...
    return write(subpkg_doc_files, map_sfi_to_docfile)
