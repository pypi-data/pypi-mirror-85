...
def calculate_ages_to_modify_and_counter(
    ...
    * `comp_in_comp_end`: a bool; is the `comp` in `self.get_J(comp)` the
    ...
    max_of_either = max(-1, to_modify_age_int - age_down_to)
    to_counter_age_min = min(age_floor, max_of_either) if use_age_floor \
        else max_of_either
...
def calculate_counter_adjustable_in_comp(
    ...
    for pop_end_minus_at_least_val, comp_val in zippc:
        if pop_end_minus_at_least_val <= 0:
            ...
            # be the "end-of-period pop of that record - 1" REGARDLESS of
            # the value at the problematic component. 
            # This is because:
            #     1. if the negative component increases more than that, 
            #         then the "end-of-period population - 1" of the 
            #         neighbouring age record will go below 1.
            #     2. all the values in the population table is (supposedly)
            #         nonnegative, and a counter-adjustment to negative 
            #         problematic components will increase the component 
            #         value at that record (e.g. death value 2 -> 3). There
            #         is no limit as to how much it can increase so long as
            #         the "end-of-period pop of that record - 1" remains
            #         nonnegative.
            ...
            # the "end-of-period population - 1" or the component value 
