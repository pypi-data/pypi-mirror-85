...
import time
try:
    from neutron_arista_ccf_lldp.rhlib import get_network_interface_map
except ImportError:
    pass
...
def chassis_id_tlv_of(chassis_id, subtype=CHASSIS_ID_LOCALLY_ASSIGNED):
    return tlv_of(
        1, raw_bytes_of_int(subtype, 1, "Chassis ID subtype") + chassis_id.encode())


def port_id_tlv_of(port_id, subtype=PORT_ID_INTERFACE_ALIAS):
    return tlv_of(2, raw_bytes_of_int(subtype, 1, "Port ID subtype") + port_id.encode())
...
def _send_frames_kernel_socket(network_map, br_bond_name, args):
    ...
    systemname = hostname_fqdn + '_' + br_bond_name.decode('utf-8')
    LOG.syslog("LLDP system-name is %s" % systemname)
