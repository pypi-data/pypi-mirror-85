...
class LdapService(object):
    ...
    def get_user(self, username, connection=None, raise_error=False):
        user_entry = self.get_user_entry(username, connection, raise_error)
        if not user_entry:
            return None
        return self.entry_to_json(user_entry)

    def get_user_entry(self, username, connection=None, raise_error=False):
    ...
    def get_users(self, paged_size=200, connection=None, raise_error=False):
        entries = self.get_user_entries(paged_size=paged_size, connection=connection, raise_error=raise_error)
        return list(map(self.entry_to_json, entries))

    def get_user_entries(self, paged_size=200, connection=None, raise_error=False):
        entries = []
        connection = connection or self.get_connection()
        extra_params = {}
        counter = 0
