...
This file contains a black box interface and two implementations: for
uPDK file and for GDS file. Also black box validator is included.
...
from .updk import DEFAULT_BB_METAL_OUTLINE_LAYER
...
from .updk import BBox, BbMetalOutline
...
MAPPING = {
    ...
    "bb_metal_outline": [DEFAULT_BB_METAL_OUTLINE_LAYER, 0],
}
...
def is_gds_mapping_valid(
        mapping, raise_error=False, full_output=False, version='0.2'
    ):
...
class _BlackBox(ABC):
    ...
    @property
    @abstractmethod
    def bb_metal_outline(self) -> Optional["BbMetalOutline"]:
        raise NotImplementedError("")
    ...
    def __eq__(self, other: "_BlackBox") -> bool:
        ...
        metal_outline = self.bb_metal_outline == other.bb_metal_outline
    ...
    def full_info(self):
        s = (
            ...
            f"  BB metal: {self.bb_metal_outline}\n"
...
class uPDKBlackBox(_BlackBox):
    def __init__(self, bb: BB, parameter_values=None, *args, **kwargs):
        ...
        self._bb_metal_outline = None
        self._bb_metal_outline_calculated = False
    ...
    @property
    def bb_metal_outline(self) -> Optional["BbMetalOutline"]:
        if not self._bb_metal_outline_calculated:
            self._bb.set_parameters(**self.parameter_values)
            self._bb_metal_outline = self._bb.bb_metal_outline
            self._bb_metal_outline_calculated = True

        return self._bb_metal_outline
...
class GdsBlackBox(_BlackBox):
    ...
    def bb_metal_outline(self) -> Optional["BbMetalOutline"]:
        if not self._bb_metal_outline_calculated:
            i = self._layout.find_layer(*self.mapping["bb_metal_outline"])
            if i is None:
                # No layer = no metal pads
                bb_metal_outline = None
            elif self._cell.shapes(i).size() == 0:
                bb_metal_outline = None
            else:
                poly_pts = []
                for s in self._cell.shapes(i).each():
                    if s.is_box():
                        sw = (self._to_um(s.box_p1.x), self._to_um(s.box_p1.y))
                        ne = (self._to_um(s.box_p2.x), self._to_um(s.box_p2.y))
                        pts = [sw, (sw[0], ne[1]), ne, (ne[0], sw[1])]
                        ...
                        raise ValueError(
                            "One of the metal pads is not a box or a simple "
                            "polygon."
                        )
                    poly_pts += [pts]

                bb_metal_outline = BbMetalOutline.from_points(poly_pts)

            self._bb_metal_outline = bb_metal_outline
            self._bb_metal_outline_calculated = True
