...
class Client:
    ...
    def query(self, input, scantimeout=3.0, params=(), **kwargs):
        """
        ...
        You can add a time limit for the query to be retrieved with

            client.query(input='pi', scantimeout=10.0)

        Longer query times are usually useful if the result takes time
        on WolframAlpha's Servers to be processed (like Integrals!).
        Note: the default value is 3.0 as for 3 seconds.
        """
        ...
        url = 'https://api.wolframalpha.com/v2/query?' + query + '&scantimeout=' + str(scantimeout)
