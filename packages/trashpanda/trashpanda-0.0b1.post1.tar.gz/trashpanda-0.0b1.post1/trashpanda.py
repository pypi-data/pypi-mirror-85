...
def add_missing_indexes_to_series(
    ...
    Examples:
        ...
        >>> new_indexes = Index(list(iter("ad")))
        >>> add_missing_indexes_to_series(target, new_indexes)
        a       1
        b       1
        c       1
        d    <NA>
        Name: foo, dtype: object
        >>> add_missing_indexes_to_series(target, new_indexes, "X")
        a    1
        b    1
        c    1
        d    X
...
def get_intersection_of_series(
    full_series: Series, targeted_indexes: pandas.Index
) -> Series:
    """
    ...
    *full_series* is made for the *targeted_indexes*, which must not necessaraly
    ...
    """
    existing_indexes = full_series.index
    possible_indexes = existing_indexes.intersection(targeted_indexes)
    requested_series = full_series.loc[possible_indexes]
