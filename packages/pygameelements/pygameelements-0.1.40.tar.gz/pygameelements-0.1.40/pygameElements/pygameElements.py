        ...
        if self.sysFont: # using system font
            ...
            if os.path.isfile(self.fontName): # try loading it from the same location as the program 
                ...
                if os.path.isdir('fonts') and os.path.isfile(os.path.join('fonts' , self.fontName)):
                    ...
                    parts=os.path.abspath(__file__+"/../..")
                    print(f"parts={parts}")
                    parts=os.path.join(parts , 'pygameElementsTest')
                    folder=os.path.join(parts , 'fonts')
                    fileLocation=os.path.join(folder , self.fontName)
                    if os.path.isdir(folder) and os.path.isfile(fileLocation):
                        self.basicFont = pygame.font.Font(fileLocation, self.fontSize) 
