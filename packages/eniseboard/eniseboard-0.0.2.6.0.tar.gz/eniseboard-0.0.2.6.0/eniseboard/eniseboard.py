...
def cloneMode(method):
    ...
    def wrapped(self,*args,**kwargs):
        if self.clone and not self.started and not method.__name__=='console':
            ...
            if method.__name__=='getSelection':
                return []
            else:
                return
        else:
            ...
            if broadcast and self.clone:
                # l'appel ne vient pas du réseau => il faut le broadcaster
                if method.__name__ == 'console':
                    name = 'boardconsole'
                else:
                    name = method.__name__
                self.conn.broadcast(name,[args,kwargs])
...
class eniseboard():
    ...
    def __init__(self,**kwargs):
        ...
        if not self.clone:
            ...
            with connexion(self.__master) as self.conn:
                ...
                self.conn.on('getSelection',self.__getSelection_msg)
    ...
    def __getSelection_msg(self,player,pseudo,data):
        """ réception d'un message réseau """
        self.getSelection(*data[0],**data[1],broadcast=False)
