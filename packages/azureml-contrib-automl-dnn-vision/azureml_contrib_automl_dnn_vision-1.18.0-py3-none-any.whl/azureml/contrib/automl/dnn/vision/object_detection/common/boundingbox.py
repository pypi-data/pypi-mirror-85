...
class BoundingBox(json.JSONEncoder):
    ...
    def __init__(self, label, box, score, mask=None):
        """
        ...
        :param score: Score between 0 and 1 of confidence in box and classification, 0 being no confidence and 1 being
                      complete confidence.
        :type score: float
        :param mask (optional): Bitmask associated with this instance
        :type mask: HxW Tensor (or None)
        """
        self._label = label
        self._score = score
        self._x = box[0]
        ...
        self._mask = (mask > 0.5) if mask is not None else None
    ...
    @property
    def mask(self):
        """Get the bit mask of object instance, if present.

        :return: Bitmask of the object instance or None
        :rtype: height x width unit8 numpy array or None
        """
        return self._mask
...
class ImageBoxes:
    ...
    #  'masks' (optionally): NxHxW Bitmasks for instance segmenation (if moodel supports)
    ...
    def add_boxes(self, boxes, labels, scores, masks=None):
        ...
        :param masks (optional): Bitmasks for instance segmenation
        :type masks: NxWxH Tensor Array (or None)
        """

        self._has_mask = masks is not None

        if not self._has_mask:
            masks = [None] * len(boxes)

        for box, label, score, mask in zip(boxes,
                                           labels,
                                           scores,
                                           masks):
