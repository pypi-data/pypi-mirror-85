...
def install():
  ...
  if not error:
    typer.secho(f"Installation successful. Use 'backplane start' to get going.", err=False, fg=typer.colors.GREEN)
  else:
    typer.secho(f"Installation failed. Use 'backplane -v 1 install' for verbose output.", err=False, fg=typer.colors.GREEN)
...
def runCommand(
  ...
  try:
    ...
    result = subprocess.run(compose_command, shell=True, cwd=project_dir, capture_output=True)
    return result
  except Exception as e:
