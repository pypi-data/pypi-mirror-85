...
class Elastic:
    ...
    def log_line(self, index: str, timestamp: str, information: dict, replace_on: list = None):
        """
        Write a line to the elasticsearch database
        :param index: the name of the database
        ...
        :param replace_on: the new document will replace documents with the same values in these columns
        :return: the response of the post request
        """
        # Look for template with matching name
        url = f'{self.host}/_template/{index}?pretty'
        response = requests.get(url, headers=self.headers)
        if response.status_code >= 400:
            return f'Cannot find an index template for index \'{index}\' with the same name.'
        template = response.json()
        template_properties = template[index]['mappings']['properties']
        ...
        index_name = f'{index}-{created_at[:10]}'
        url = f'{self.host}/{index_name}'
        response = requests.get(url=url, headers=self.headers)
        if response.status_code >= 400:
            self.create_index(index_name)
        # If the new document should replace old documents: first remove the old documents
        if replace_on is not None:
            # Search for old documents
            url = f'{self.host}/{index}-*/_search'
            match = {}
            for col in replace_on:
                match[col] = information[col]
            ...
            for i in range(len(df)):
                url = f'{self.host}/{df["_index"][i]}/_doc/{df["_id"][i]}'
