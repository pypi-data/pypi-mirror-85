...
class TaskScheduler(object):
    ...
    def __init__(self, host, user, password, database, task_id, loglevel='INFO', email_after_errors=False,
        ...
        if es_host is not None:
            self.es = Elastic(es_host, es_port, es_user, es_password)
        else:
            self.es = None
    ...
    def check_if_logging_tables_exists(self):
        ...
        # If logging to Elastic Search is enabled, create index template 'task_execution_log' if it doesn't exist yet
        if self.es is not None:
            template_name = 'task_execution_log'
            index_template = {
    ...
    def write_execution_log(self, message: str, loglevel='INFO'):
        ...
        if loglevel not in allowed_loglevels:
            ...
            replace_on = ['log_level']
            if self.loglevel == 'DEBUG':
                # Count the errors
                if loglevel == 'ERROR' or loglevel == 'CRITICAL':
                    self.error_count += 1
                if self.es is not None:
                    self.es.log_line('task_execution_log', timestamp, information, replace_on)
    ...
    def error_handling(self, e: Exception, breaking=True, send_to_teams=False):
        ...
        if breaking:
            ...
            if self.es is not None:
                self.es.log_line('task_execution_log', timestamp, information)
