...
def InstaWeb_GetData(self,Link,Private=False):
    ...
    r.close()
    return content
def InstaWeb_Download(self, Filename, Data):
...
def InstaWeb_GetUrl(self, Data):
    ...
    if content['__typename'] == "GraphImage":
        ...
        for x in content['edge_sidecar_to_children']['edges']:
            urls.append(x)
        return urls
def InstaMobile_GetData(self, Link, Private=False):
    ...
    r.close()
    return content
def InstaMobile_Download(self, Filename, Data):
    content = Data['graphql']['shortcode_media']
    if content['__typename'] == "GraphImage":
        r = requests.get(content['display_url'])
        with open(Filename + ".png", "wb") as f:
        ...
        for x in content['edge_sidecar_to_children']['edges']:
            if x['node']['__typename'] == "GraphVideo":
                ...
                with open(Filename + ".png", "wb") as f:
                    f.write(r.content)
    r.close()
def InstaMobile_GetUrl(self, Data):
    content = Data['graphql']['shortcode_media']
    if content['__typename'] == "GraphImage":
        return content['display_url']
    elif content['__typename'] == "GraphVideo":
        ...
        for x in content['edge_sidecar_to_children']['edges']:
            urls.append(x)
        return urls
def SnapChat_GetData(self, Username):
    rHeaders = {
    ...
    content = json.loads(r.content)
    return content
def SnapChet_Download(self, Filename, Data):
    content = Data
    for index,x in enumerate(content['story']['snaps'], start=1):
        with open(str(index) + "-" + Filename + ".mp4" if x['media']['type'] == "VIDEO" else ".png", "wb") as f:
            ...
            f.write(r.content)
def SnapChat_GetUrl(self, Data):
    urls = []
    content = Data
    for url in content['story']['snaps']:
        urls.append(url)
    return urls
def Twitter_GetData(self, Link):
    code = Link.split("/")[-1]
    rHeaders = {
        "Accept": "*/*",
    ...
    content = json.loads(r.content)
    return [content, code]
def Twitter_Download(self, Filename, Data):
