...
def InstaWeb_GetData(Link,Private=False):
    ...
    r.close()
    return content
def InstaWeb_Download(Filename, Data):
...
def InstaWeb_GetUrl(Data):
    ...
    if content['__typename'] == "GraphImage":
        ...
        for x in content['edge_sidecar_to_children']['edges']:
            urls.append(x)
        return urls
def InstaMobile_GetData(Link, Private=False):
    ...
    r.close()
    return content
def InstaMobile_Download(Filename, Data):
    content = Data['graphql']['shortcode_media']
    if content['__typename'] == "GraphImage":
        r = requests.get(content['display_url'])
        with open(Filename + ".png", "wb") as f:
...
def SnapChat_GetData(Username):
    ...
    content = json.loads(r.content)
    return content
def SnapChet_Download(Filename, Data):
    content = Data
    for index,x in enumerate(content['story']['snaps'], start=1):
        with open(str(index) + "-" + Filename + ".mp4" if x['media']['type'] == "VIDEO" else ".png", "wb") as f:
            ...
            f.write(r.content)
def SnapChat_GetUrl(Data):
    urls = []
    content = Data
    for url in content['story']['snaps']:
        urls.append(url)
    return urls
def Twitter_GetData(Link):
    code = Link.split("/")[-1]
    rHeaders = {
        "Accept": "*/*",
    ...
    content = json.loads(r.content)
    return [content, code]
def Twitter_Download(Filename, Data):
