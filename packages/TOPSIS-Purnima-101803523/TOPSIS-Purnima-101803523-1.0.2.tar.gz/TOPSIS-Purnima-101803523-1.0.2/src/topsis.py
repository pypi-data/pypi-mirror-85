# Purnima Lal | 101803523 | UCS538 | Assignment-06
...
def main():
    ...
    if len(sys.argv) < 5:
        print("wrong number of parameters passed. Correct parameters-(inputFileName, Weights, Impacts, resultFileName)")
        sys.exit()
    ...
    try:
        ...
        print("Weights or Impacts are not provided in correct format")
        sys.exit()
        
    for each in impacts :
        if each not in ('+','-'):
            print("Impacts are not provided in correct format. Impacts must be either +ve or -ve.")
            sys.exit()
    ...
    print ("Result file saved in working directory")
