...
def display():
    ...
    data_ori,data_res,all_used_cols,res_used_cols = DB.get_table(file_name,round_n=num)
...
def python_code():
    ...
    for u in updates:
        ...
        if str(u['gr_val']).isdigit():
            ...
            gr_val = u['gr_val']
        final_part[id_val]=gr_val
    #codes = "df[raw]=df[grouping].map('+final+')"
    final_all = {}
    file_name = DB.get_file_name()
    final_all = DB.get_col_map(file_name,col1,col2)

    if not final_part:
        final_part = "no updates"
        final_all = "no updates"
    return render_template("python_code.html",final_part = final_part, \
                            final_all = final_all, col1=json.dumps(col1),col2=json.dumps(col2))
...
def pyspark_code():
    ...
    if not final_part:
        final_part = "no updates"
        final_all = "no updates"
    return render_template("pyspark_code.html",final_part = final_part, final_all=final_all,\
            col1=json.dumps(col1),col2=json.dumps(col2))
...
def do_update(data):
    ...
    data_res,res_used_cols = DB.get_table(file_name,update=True,round_n=num)
    ajax_df_res = json.dumps(data_res.to_dict('records'))
    res_used_cols = json.dumps(res_used_cols)
