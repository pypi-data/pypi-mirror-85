...
class DBHelper:
    ...
    def get_table(self,file_name,update=False,round_n=2):
        params = self.get_params()
        ...
        #get two tables
        df_cal = self.helper_cal(df_res.copy(),params['cnt_ft'],params['avg_ft'],round_n)
        res_used_cols = df_cal.columns.tolist()

        if not update:
            df_ori = self.helper_cal(df.copy(),params['cnt_ft'],params['avg_ft'],round_n)
