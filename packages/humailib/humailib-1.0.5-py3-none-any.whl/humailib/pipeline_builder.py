...
class PipelineBuilder:
    ...
    def build(self, pipelines=None, catch_all_params=None, params_append_to_existing=False, verbose=False):
        ...
        if catch_all_params is not None:
            for catch_all_name, catch_all_value in catch_all_params.items():
                for current_pipeline in pipelines:
                    ...
                    if params_append_to_existing:
                        for name,val in components_params.items():
                            if catch_all_name in name and isinstance(val, list):
                                params[name] = list(set(val)+set(catch_all_value))
                    if verbose:
                        print("++++ Matching component params:")
