...
"""
The PublisherHealthCheck class defines the readiness interface for
a :py:class:`~olace.messaging.publisher.message_publisher.MessagePublisher` object.
"""
...
class PublisherHealthCheck(ABC):
    ...
    def is_ready(self) -> bool:
        """
        Check if a publisher can publish messages.

        Returns:
            bool: True if publisher can publish messages. False if publisher is prevented from sending messages including (e.g., full buffer or I/O problems).
        """

    @abstractmethod
    def set_publisher_readiness_listener(self, listener: 'PublisherReadinessListener'):
        """
        Set  a :py:class:`solace.messaging.publisher.publisher_health_check.PublisherReadinessListener`.
    ...
    def notify_when_ready(self):
        """
            Request to notify the application when the publisher is ready.
            ...
            logic to handle the ``PublisherOverflowError``, applications can call :py:meth:`PublisherHealthCheck.notify_when_ready()` to be
            guaranteed to receive a subsequent :py:meth:`PublisherReadinessListener.ready()` callback.
        """
...
class PublisherReadinessListener(ABC):  # pylint: disable=too-few-public-methods
    ...
    def ready(self):
        """
        This method is executed when the publisher is in a state when it can publish.
