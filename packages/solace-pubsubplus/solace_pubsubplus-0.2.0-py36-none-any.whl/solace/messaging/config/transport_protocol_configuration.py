...
class TransportProtocolConfiguration(ABC):
    """This is an abstract class which provides the configuration for transport layer.
    ...
    def with_message_compression(self, compression_factor: int) -> 'TransportProtocolConfiguration':
        ...
        Args:
            compression_factor (int): Enables messages to be compressed with ZLIB before transmission and decompressed on receive. The valid  values to use are
             0 (off) or 1..9, where 1 is least amount of compression (fastest) and 9 is the most amount of compression (slowest).

        Returns:
            TransportProtocolConfiguration: Instance of the transport configuration that can be used for method chaining.
        """
    ...
    def with_connection_retry_strategy(self, strategy: 'RetryStrategy') -> 'TransportProtocolConfiguration':
        ...
        connection attempt fails.  If an established connection fails, see :py:meth:`TransportProtocolConfiguration.with_reconnection_retry_strategy`.
