...
from solace.messaging.config.receiver_property_configuration import ReceiverPropertyConfiguration
from solace.messaging.resources.topic_subscription import TopicSubscription


class MessageReceiverBuilder(ReceiverPropertyConfiguration):  # pylint: disable=missing-class-docstring
    """
    A  abstract base class for builders for message receivers.
    """

    @abstractmethod
    def with_subscriptions(self, subscriptions: List[TopicSubscription]) -> \
            'MessageReceiverBuilder':  # pylint: disable=missing-function-docstring
        """
    ...
    def from_properties(self, configuration: dict) \
            -> 'MessageReceiverBuilder':  # pylint: disable=missing-function-docstring
        """
        Sets the properties for direct message receiver (``DirectMessageReceiver``) using a dictionary. The dictionary is
        comprised of tuples.
