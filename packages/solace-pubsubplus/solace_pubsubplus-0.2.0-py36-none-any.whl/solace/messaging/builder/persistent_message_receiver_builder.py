...
"""
 ...
 Applications that need to receive persistent messages must first create a ``PersistentMessageReceiverBuilder`` instance
 using the the :py:meth:`MessagingService.create_persistent_message_receiver_builder()` method.
...
class PersistentMessageReceiverBuilder(MessageReceiverBuilder,
    ...
    """
    This class is used to configure and create instances of ``PersistentMessageReceiverBuilder``.
    The ``PersistentMessageReceiverBuilder`` builds the :py:class:`solace.messaging.receiver.persistent_message_receiver.PersistentMessageReceiver` instances.
    """
    ...
    def with_message_selector(self, selector_query_expression: str) -> 'PersistentMessageReceiverBuilder':
        ...
        When a selector is applied, then the receiver only gets messages whose headers and properties match the selector.
    ...
    def build(self, endpoint_to_consume_from: Queue) -> PersistentMessageReceiver:
        ...
        :py:class:`solace.messaging.publisher.persistent_message_receiver.PersistentMessageReceiver`.

        Args:
            endpoint_to_consume_from(Queue): The ShareName as the shared group name.
