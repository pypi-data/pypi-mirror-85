...
""" Module contains the classes and functions to work with Queues."""
...
class Queue(NamedResource, ResourceDurability, ResourceAccessibility, ABC):  # pylint: disable=too-few-public-methods
    ...
    """
    An interface that abstracts a PubSub+ broker resource used primarily for receiving messages.
    ...
    @staticmethod
    def non_durable_exclusive_queue(queue_name: str) -> 'Queue':
        """
        Create an exclusive tempoary queue for non-durable consumers. More
        ...
        Args:
            queue_name (str): The name of the temporary, exclusive queue.
...
class Queues:
    """class contains different types of Queues"""
    ...
    class SimpleDurableQueue(DurableQueue):
        ...
        def __str__(self):
            return "SimpleDurableQueue{name=\'" + self._name + "\',  exclusively_accessible : " + \
                   str(self._exclusively_accessible) + "}"

    class SimpleNonDurableQueue(NonDurableQueue):
        """A Simple non-durable queue."""
        ...
        def __str__(self):
            return "SimpleNonDurableQueue{name=\'" + self._name + "\',  exclusively_accessible : " + \
                   str(self._exclusively_accessible) + "}"
