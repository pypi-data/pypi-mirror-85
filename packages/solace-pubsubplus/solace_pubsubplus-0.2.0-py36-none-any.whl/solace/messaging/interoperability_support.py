...
The PubSub+ Event Broker supports many messaging protocols.  When acting as a gateway
...
class RestInteroperabilitySupport(ABC):
    ...
    def get_http_content_type(self) -> str:
        """
        Retrieve the HTTP content type header value from message originally published by a REST client.
    ...
    def get_http_content_encoding(self) -> str:
        """
        Retrieve the HTTP content encoding value from message originally published by a REST client.
...
class InteroperabilitySupport(ABC):  # pylint: disable=too-few-public-methods
    ...
    def get_rest_interoperability_support(self) -> 'RestInteroperabilitySupport':
        """
         Retrieve access to the optional metadata used for interoperability with REST messaging clients.
