...
class BaseFromRaw(BaseIO):
    ...
    supported_objects = [Block, Segment, AnalogSignal,
                         SpikeTrain, Group, ChannelView, Event, Epoch]
    ...
    def read_segment(self, block_index=0, seg_index=0, lazy=False,
        ...
        if signal_channels.size > 0:
            channel_index_list = self.get_group_signal_channel_indexes()
            for channel_index in channel_index_list:
                for ind_within in self._make_signal_channel_subgroups(
                        channel_index, signal_group_mode):
                    ind_abs = channel_index[ind_within]
                    # make a proxy...
    ...
    def _make_signal_channel_subgroups(self, channel_index,
                                       signal_group_mode='group-by-same-units'):
        """
        For some RawIOs channels are already split into groups,
        but in any case, channels need to be split again into subgroups
        because they do not have the same physical units (e.g. volts vs amps)

        They can also be split one by one to match the previous behaviour for
        some IOs in older versions of Neo (<=0.5).

        This method returns indices that can be used to either (i) aggregate signal
        channels with the same physical units or (ii) split all channels.
        """
        all_channels = self.header['signal_channels']
        if channel_index is None:
            channel_index = np.arange(all_channels.size, dtype=int)
        ...
        if signal_group_mode == 'group-by-same-units':
            ...
            for unit in all_units:
                ...
                index_groups.append(ind_within)
        elif signal_group_mode == 'split-all':
            for i, chan_index in enumerate(channel_index):
                ind_within = [i]
                index_groups.append(ind_within)
        else:
