...
with open('pdf2txt/colors.csv', mode='r') as infile:
    reader = csv.reader(infile, delimiter='\t')
    next(reader)
    for row in reader:
        RGB=np.append(RGB, np.array([eval('['+row[1]+']')]), 0)
        HexNameDict['#'+row[2]]=row[0]


#RGB=np.array(RGB)
#RGB = RGB.reshape((RGB.shape[0], 1))

def get_color_name(rgb, exact=True):

     if exact:
        color_rgb=rgb
     else:
        color_rgb=(RGB[spatial.KDTree(RGB).query(rgb)[1]])


     s = '#' \
            + format(color_rgb[0], 'x').zfill(2) \
            + format(color_rgb[1], 'x').zfill(2) \
            + format(color_rgb[2], 'x').zfill(2)
     ColorHex = s.upper()  # "#8B7355"  # "#8B7355"
     ColorDiff = \
            '(' + '{0:+d}'.format(color_rgb[0] - rgb[0]) \
            + ',' + '{0:+d}'.format(color_rgb[1] - rgb[1]) \
            + ',' + '{0:+d}'.format(color_rgb[2] - rgb[2]) \
            + ')'
     try:
