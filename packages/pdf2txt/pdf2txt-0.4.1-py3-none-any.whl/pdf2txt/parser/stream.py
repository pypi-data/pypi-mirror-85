...
class Stream():
    ...
    def _add_columns(cols, text, row_tol):
        ...
        if text:
            ...
            tokens = [len(r) for r in text]
            new_cols = [
                (t.x0, t.x1) for r in text if len(r) == max(tokens) for t in r
            ]
    ...
    def _generate_columns_and_rows(self, table_idx, tk):
        # select tokens which lie within table_bbox
        t_bbox = {}
        ...
        tokens = [len(r) for r in rows_grouped]
        ...
        if self.columns is not None and self.columns[table_idx] != "":
            ...
            # calculate mode of the list of number of tokens in
            # each row to guess the number of columns
            ncols = max(set(tokens), key=tokens.count)
            if ncols == 1:
                # if mode is 1, the page usually contains not tables
                # but there can be cases where the list can be skewed,
                # try to remove all 1s from list in this case and
                # see if the list contains tokens, if yes, then use
                # the mode after removing 1s
                tokens = list(filter(lambda x: x != 1, tokens))
                if len(tokens):
                    ncols = max(set(tokens), key=tokens.count)
                else:
                    warnings.warn(
