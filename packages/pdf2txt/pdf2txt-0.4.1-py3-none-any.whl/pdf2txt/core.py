    ...
    def update(self, textline):
        ...
        for align in ["left", "right", "middle"]:
            ...
            if idx is None:
                ...
                self._textedges[align][idx].pdflines.append(textline.Text)
...
#                te.is_valid = False
            if te.is_valid:
                if nb_overlap(lines, te)>=2 or overlap([span for tl in textlines for span in tl.tokens], te):
                    te.is_valid=False
        ...
        for line_edge in relevant_textedges:
            found=False
            while not overlap([span for tl in textlines for span in tl.tokens], line_edge) and line_edge.y0>0 and not overlap(lines, line_edge):
                line_edge.y0-=5
                found=True
            if found:
                line_edge.y0+=15
            ...
            found=False
            while not overlap([span for tl in textlines for span in tl.tokens], line_edge) and line_edge.y1 < self.page_height and not overlap(lines, line_edge):
                line_edge.y1 += 5
                found=True
            if found:
                line_edge.y1 -= 15
