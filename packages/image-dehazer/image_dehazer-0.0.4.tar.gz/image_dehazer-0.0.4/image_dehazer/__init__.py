...
class image_dehazer():
    def __init__(self, airlightEstimation_windowSze=15, boundaryConstraint_windowSze=3, C0=20, C1=300, regularize_lambda=0.1, sigma=0.5, delta=0.85):
    ...
    def __BoundCon(self, HazeImg):
        if (len(HazeImg.shape) == 3):
            ...
            t_b = np.maximum((self._A[0] - HazeImg[:, :, 0].astype(np.float)) / (self._A[0] - self.C0),
                             (HazeImg[:, :, 0].astype(np.float) - self._A[0]) / (self.C1 - self._A[0]))
            t_g = np.maximum((self._A[1] - HazeImg[:, :, 1].astype(np.float)) / (self._A[1] - self.C0),
                             (HazeImg[:, :, 1].astype(np.float) - self._A[1]) / (self.C1 - self._A[1]))
            t_r = np.maximum((self._A[2] - HazeImg[:, :, 2].astype(np.float)) / (self._A[2] - self.C0),
                             (HazeImg[:, :, 2].astype(np.float) - self._A[2]) / (self.C1 - self._A[2]))

            MaxVal = np.maximum(t_b, t_g, t_r)
            self._Transmission = np.minimum(MaxVal, 1)
        else:
            self._Transmission = np.maximum((self._A[0] - HazeImg.astype(np.float)) / (self._A[0] - self.C0),
                                            (HazeImg.astype(np.float) - self._A[0]) / (self.C1 - self._A[0]))
            self._Transmission = np.minimum(self._Transmission, 1)

        kernel = np.ones((self.boundaryConstraint_windowSze, self.boundaryConstraint_windowSze), np.float)
