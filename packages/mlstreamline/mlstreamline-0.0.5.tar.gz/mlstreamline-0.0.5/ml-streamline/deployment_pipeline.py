...
class DeploymentPipeline:
    def __init__(self, config, pipeline_path=None, classifier=None, numeric_transform='box-cox'):
        """
    ...
    def check_configs(self):
        columns_check = self.config['numeric_features'] + \
                ...
                self.config['no_transfrom_columns']
    ...
    def construct_preprocessor(self):
        ...
        """
        if self.numeric_transform == 'box-cox':
            numeric_transformer = Pipeline(steps=[
                            ('imputer', SimpleImputer(strategy='median')),
                            ('numeric_scaler', PowerTransformer(method='box-cox'))
                    ])
        elif self.numeric_transform == 'yeo-johnson':
            numeric_transformer = Pipeline(steps=[
                            ('imputer', SimpleImputer(strategy='median')),
                            ('numeric_scaler', PowerTransformer(method='yeo-johnson'))
                    ])
        elif self.numeric_transform == 'standard':
            numeric_transformer = Pipeline(steps=[
                            ('imputer', SimpleImputer(strategy='median')),
                            ('numeric_scaler', StandardScaler())
                    ])
        else:
            print('Available numeric_transform values are box-cox, yeo-johnson, or standard')
            assert(False)
        ...
        transformers = [
                            ...
                            ('no_transform', no_transform, self.config['no_transfrom_columns'])
                        ]
