...
def social_login(self, request):
    ...
    if provider == settings.ENUMS.SOCIAL_TYPES.Facebook:
        ...
        user_exist = User.objects.filter(user_social_identifier__identifier=user_data.get('id'))

    elif provider == settings.ENUMS.SOCIAL_TYPES.Google:
        user_exist = User.objects.filter(user_social_identifier__identifier=user_data.get('sub'))
