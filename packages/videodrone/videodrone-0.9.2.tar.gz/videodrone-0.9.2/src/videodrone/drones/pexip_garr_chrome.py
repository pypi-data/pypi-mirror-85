...
def run(room='videodrone', y4m='./y4m', lifetime=360, headless=1, pin=None):
    ...
    time.sleep(3)
    browser.find_element_by_id('display-name-dialog-input').send_keys('videodrone')
    time.sleep(1)
    ...
    time.sleep(lifetime)
    # leave the room
    browser.find_element_by_xpath('//*[@id="toolbar-disconnect__button-container"]/button').click()
    try:
        browser.close()
