...
class Users(object):
	...
	def update(self,
		...
		try:
			user = auth.update_user(
				...
				disabled=False)
			success = True
		except Exception as error: success = False

		# handle success.
		if success:
			return r3sponse.success_response(f"Successfully updated user [{username}].")
		else:
			return r3sponse.error_response(f"Failed to update user [{username}], error: {error}.")
	...
	def verify_id_token(self, id_token):
		...
		if success:
			...
			return r3sponse.error_response("You are not signed in., error: {error}.")
