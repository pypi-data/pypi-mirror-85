...
class Users(object):
	...
	def send_code(self, 
		...
		# send email.
		html = utils.__load_file__(html).format(
			code=code,
			client=user.display_name,
			ip=ip,
			timestamp=Formats.Date().timestamp,
			white=TEMPLATE_DATA["colors"]["white"],
			dark_grey=TEMPLATE_DATA["colors"]["dark_grey"],
			purple=TEMPLATE_DATA["colors"]["purple"],
			grey=TEMPLATE_DATA["colors"]["grey"],
			black=TEMPLATE_DATA["colors"]["black"],
			tint=TEMPLATE_DATA["colors"]["backgrounds"]["top_bar"],)
