...
from typing import TYPE_CHECKING, List, Any, Tuple, cast, Dict
if TYPE_CHECKING:
...
class ChartClassStyle:
    """ The data that determine the visual style of a class. """
    def __init__(self,
        group_name : str = "",
    ...
    @staticmethod
    def from_json(json : Dict[str, Any]) -> "ChartClassStyle":
        assert json.pop("type") == ChartClassStyle.__name__
        return ChartClassStyle(**json)

    @property
    def group_name(self) -> str:
        """ The name of the "group" represented by the current glyph. This does not directly affect the rendered chart, 
            it is intended for clerical purposes. If group_name is present it is treated as if it were unique by the ClassStyle repr.
        """        
...
class ChartClass:
    ...
    def set_style(self, page : slice, style : ChartClassStyle) -> "ChartClass":
    ...
    def replace(self, 
        ...
        self._max_page = INFINITY
        if style:
            self.set_style(slice(page, None), style)
        for [key, value] in [
            ["shape", shape], 
