...
def start_task(name: str, retention_days: int = None):
    ...
    if not inited:
        logging.getLogger(__name__).error('Trying to create a new timbermill task without it being inited, ignoring..')
        return TimberLogNotInitedContext()
    else:
        return TimberLogContext(name, retention_days)


class TimberLogNotInitedContext:
...
def spot(name: str, context: dict = None, strings: dict = None, metrics: dict = None, text: dict = None, parent_id: str = None, task_success: bool = True) -> str:
    global inited
    if not inited:
        logging.getLogger(__name__).error('Trying to create a new timbermill spot event without it being inited, ignoring..')
        return None
