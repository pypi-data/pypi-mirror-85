...
def oldest_album(artist_name):
    ...
    with closing(sqlite3.connect("itunes.db")) as connection:
        with closing(connection.cursor()) as cursor:
            rows = cursor.execute("""SELECT collectionName, releaseDate FROM albums WHERE artistName = ? ORDER BY releaseDate ASC LIMIT 1""", (artist_name,)).fetchall()
            return rows
...
def average_price(artist_name):
    ...
    with closing(sqlite3.connect("itunes.db")) as connection:
        with closing(connection.cursor()) as cursor:
            prices = [row[0] for row in cursor.execute("SELECT collectionPrice FROM albums WHERE artistName = ?", (artist_name,)).fetchall()]
