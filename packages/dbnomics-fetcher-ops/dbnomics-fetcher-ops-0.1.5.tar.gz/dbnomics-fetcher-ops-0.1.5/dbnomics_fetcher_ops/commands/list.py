...
def list_command(
    ...
    for provider_slug, fetcher_metadata in sorted(app_args.fetchers_metadata.items()):
        if legacy_pipeline is not None and fetcher_metadata.get("legacy_pipeline", False) != legacy_pipeline:
            continue
        if slug:
            typer.echo(provider_slug)
        else:
            typer.echo(fetcher_metadata["provider_code"])
