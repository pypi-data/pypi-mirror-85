...
def find_deploy_key_by_title(title: str) -> Optional[DeployKey]:
    for key in gitlab.GitLab.deploykeys.list(as_list=False):
        if key.title == title:
            return key
    return None
...
def ensure_ssh_key_pairs(
    ...
    if not source_data_deploy_key or not json_data_deploy_key:
        ...
        deploy_key = find_deploy_key_by_title(deploy_key_title)
