from mhclovac.sequence import sequence_to_features, validate_sequence, chop_sequence, read_fasta
...
def predict(sequence: str, peptide_length: int, mhc: str, sequence_name: str = "unknown") -> pd.DataFrame :
    """
    Predicts binding and epitope score. Returns pandas DataFrame.

    :param sequence: string, sequence string must be longer than peptide_length
    :param peptide_length: int,
    ...
    data = pd.DataFrame()
    data['sequence'] = chop_sequence(sequence, peptide_length)
    data['mhc'] = mhc
...
def run():
    ...
    if args.fasta:
        for seq_name, sequence in read_fasta(args.fasta):
            try:
                ...
                preds = predict(sequence=sequence, peptide_length=args.peptide_length, mhc=args.mhc, sequence_name=seq_name)
    ...
    if args.sequence:
        try:
            ...
            preds = predict(sequence=args.sequence, peptide_length=args.peptide_length, mhc=args.mhc, sequence_name=args.sequence_name)
            predictions.append(preds)
        except Exception as e:
