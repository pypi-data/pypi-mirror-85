...
class observation():
    ...
    def apply_POM(self):
        ...
        if self.POM_mask is None:
            ...
            POM[POM>0] = 1
            POM[POM<1] = 0
            POM = POM.astype(np.int)

            from astropy.io import fits
#            self.seg = self.seg + 1 # for testing
            ...
            fits.writeto("POM.fits",POM,overwrite=True)
    ...
    def create_pixel_list(self):
        ...
        for dir_image in self.dir_images:
            ...
            if self.SED_file==None:
                self.fs[l] = []
                dnew = d * self.POM_mask # Apply POM transmission mask to the data pixels
                ...
                for ID in self.IDs:
                    ...
                    if self.POM_mask is not None:
                        ...
                        dnew = d * self.POM_mask # Apply POM transmission mask to the data pixels. This is a single grey correction for the whole object.
