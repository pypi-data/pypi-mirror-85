...
/** Compute the one-particle density matrix of a particular state at the ADC level of
 * theory given by the method_ptr.
 *
 * \param method                The ADC method to use
 * \param ground_state_ptr      The MP ground state to build upon
 * \param excitation_amplitude  The excitation amplitude, given as a std::vector of blocks
 * (singles, doubles, ...)
 * \param intermediates_ptr     Adc intermediates to reuse during property calculation
 */
std::shared_ptr<OneParticleOperator> compute_state_diffdm(
