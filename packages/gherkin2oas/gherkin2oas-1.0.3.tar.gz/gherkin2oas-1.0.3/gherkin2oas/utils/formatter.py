...
def generate_swagger(model, security):
    ...
    temp_paths = copy.deepcopy(paths_object) # temp copy because the paths_object is being used in the below loop and also being changed at the same time
    old_paths = temp_paths.keys()
    for path in old_paths:
        new_path = re.sub(r'([^/]+)/{id}', rep, path)
        for operation in temp_paths[path]:
            for i,parameter in enumerate(temp_paths[path][operation]['parameters']):
