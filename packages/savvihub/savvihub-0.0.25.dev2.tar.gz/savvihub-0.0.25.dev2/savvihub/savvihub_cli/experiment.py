...
def logs(
    ...
    experiment_logs = client.experiment_log(
        context.project.workspace.slug, context.project.slug, experiment_number, params={'withEventLog': 'true'}, raise_error=True)
...
def run(
    ...
    if git_ref_arg:
        ...
        if git_diff_arg:
            if git_diff_arg.startswith("https://") or \
                ...
                diff_file = tempfile.NamedTemporaryFile(".patch")
    ...
    if diff_file:
        ...
        diff_file_object_id = uploaded[0].id
        diff_file.close()
    ...
    res = client.experiment_create(
        ...
        git_ref=git_ref,
        git_diff_file_id=diff_file_object_id,
