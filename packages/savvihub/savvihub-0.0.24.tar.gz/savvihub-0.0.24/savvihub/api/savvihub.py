...
from savvihub.api.types import PaginatedMixin, SavviHubListResponse, SavviHubFileObject, SavviHubDataset, \
    ...
    SavviHubExperimentLog, SavviHubSnapshot, SavviHubVolume
from savvihub.common.constants import API_HOST, WEB_HOST
...
class SavviHubClient:
    ...
    def volume_file_list(self, volume_id, *, snapshot='latest', prefix='', recursive='false', **kwargs) -> typing.List[SavviHubFileObject]:
        r = self.get(f'/v1/api/volumes/{volume_id}/files/', params={'prefix': prefix, 'recursive': recursive, 'snapshot': snapshot}, **kwargs)
    ...
    def experiment_token_read(self, **kwargs):
        resp = self.get(f'/v1/api/experiments/', **kwargs)
        if resp.status_code != 200:
            return None
        return SavviHubExperiment(resp.json())

    def experiment_read(self, workspace, project, experiment_number, **kwargs) -> SavviHubExperiment:
        return SavviHubExperiment(self.get(f'/v1/api/workspaces/{workspace}/projects/{project}/experiments/{experiment_number}', **kwargs).json())
    ...
    def experiment_create(self, workspace, project, image_url, resource_spec_id, git_ref, git_diff_file_id, start_command, dataset_mount_infos, env_vars, **kwargs):
        return self.post(f'/v1/api/workspaces/{workspace}/projects/{project}/experiments/', {
            ...
            'git_diff_file_id': git_diff_file_id,
