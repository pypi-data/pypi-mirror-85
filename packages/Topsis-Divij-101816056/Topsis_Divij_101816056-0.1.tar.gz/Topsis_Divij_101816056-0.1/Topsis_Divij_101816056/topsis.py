# -*- coding: utf-8 -*-
"""
Created on Fri Nov 13 21:23:57 2020
...
def checkParameter():
	parameters = sys.argv
	if(len(parameters)!=5 ):
		print("provide all the parameters...\n")
		return 0
	else:
		for filename in sys.argv[1:]:
			if(filename.endswith('.csv')):
				...
				if os.path.exists(File_path) != 1:
					print('filename %s doesnot exist in root directory!\n' %(filename))
					...
					if len(fileData.columns) <3:
						print('filename %s contains less than 3 columns\nTerminating...' %(filename))
						return 0
					else:
						...
						if (len(sys.argv[2].split(",")) == len(sys.argv[3].split(",")) and len(sys.argv[3].split(",")) == fileData.shape[1]-1):
							...
							print("Incorrect input. Length of impact,weights and number of columns(except 1) should be equal. \n Terminating...")
			else:
				print('filename : %s is not supported(must end with *.csv )\nTerminating...' %(filename))
				return 0
			return 1
