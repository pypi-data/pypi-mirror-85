...
class Script(WMLResource):
    ...
    def delete(self, asset_uid):
        ...
        if not self._ICP:
            ...
            if not self._client.WSD and self._if_deployment_exist_for_asset(asset_uid):
                raise WMLClientError(
                    u'Cannot delete scripts that has existing deployments. Please delete all associated deployments and try again')
