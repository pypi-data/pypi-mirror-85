...
from snippetlib_jl.upload_snippet import Upload_Snippet as us
...
class TestUploadSnippet(TestCase):
    ...
    def test_uploading(self):
        ...
        name_py = FILE_UPLOAD_FRONTEND_CONTENT['name']

        snippets = sl().collect_snippets()

        found = False
        # check_file_exists = self.test_if_snippet_available(name_py)
        for snippet in snippets:
            if name_py in snippet:
                found = True
                print("found in ",snippet)
                break
       
        # os.remove("File-name.py")
        # dirr = os.path.join(sl().get_current_directory(),'file-name.py')
        # os.remove(dirr)
