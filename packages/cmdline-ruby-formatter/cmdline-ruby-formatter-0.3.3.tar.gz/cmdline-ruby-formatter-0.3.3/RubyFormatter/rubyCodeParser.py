...
class RubyCodeParser:
    ...
    def check_name_of_file(self, full_name_of_folder):
        ...
        self._check_snake_case_format(name_of_file, isFile=True, isExclamationMark=False,
    ...
    def _check_foreign_class_method(self):
        ...
        def _get_words_by_point(word):
            ...
            for index in range(len(words_chars)):

                if words_chars[index] == '.':
                    find_point = True
                    continue

                if not find_point:
                    first_word += words_chars[index]

                if find_point:
                    if words_chars[index] == '(':
                        break
                    second_word += words_chars[index]
        ...
        def _get_vars_with_value_in_scope(my_scope):
            list_of_variables = []

            # part_with_annotation = RubyCodeParser._convert_list_to_string(scope, seperator=' ')

            chars = RubyCodeParser._get_chars_from_str(my_scope)
        ...
        def _fixing_vars_in_scope(my_vars):

            list_vars_in_scope = []

            for index_i in range(len(my_vars)):

                vars_chars = RubyCodeParser._get_chars_from_str(my_vars[index_i])
                ...
                while index_j < len(vars_chars):
                    if vars_chars[index_j] == '=':
                        find_rovno = True
                        break
                    var += vars_chars[index_j]
                    index_j += 1

                if find_rovno:

                    while index_j < len(vars_chars):
                        value_with_rovno += vars_chars[index_j]
                        index_j += 1

                # print(f' var = {var}')
                var = self._check_snake_case_format(var, isScreaming_case=False, isExclamationMark=False,
                                                    isQuestionMark=False, isFile=False)
        ...
        for i in range(len(self.list_of_lines)):
            ...
            for j in range(len(words)):
                ...
                if '.' in words[j] and '=' in words:
                    ...
                    new_line = start_line + ' ' + fix_name_of_class + '.' + fix_name_of_method + '(' + RubyCodeParser._convert_list_to_string(
                        scope, seperator=', ') + ')'
                    ...
                    new_line = fix_name_of_object + '.' + fix_name_of_method + '(' + RubyCodeParser._convert_list_to_string(
                        scope, seperator=', ') + ')'
    ...
    def _check_import_class(self):
        ...
        def is_include(list_of_words):
            if 'include' in list_of_words:
                return True
            else:
                return False

        def get_names_of_include(list_of_words):

            def get_list_name(my_words):
                return my_words[1:len(my_words)]

            names = get_list_name(list_of_words)
        ...
        def fix_names_of_import(list_names):
            ...
            for index in range(len(list_names)):
                list_names[index] = self._check_camel_case_format(list_names[index])
    ...
    def _check_snake_case_format(self, name_of_folder, isFile, isExclamationMark, isQuestionMark, isScreaming_case):
        ...
        if self._is_const(name_of_folder):
            del self.list_char_name[len(self.list_char_name)-5:len(self.list_char_name)]

            if self.list_char_name[len(self.list_char_name)-1] == '_':
                del self.list_char_name[len(self.list_char_name)-1]
        ...
        for i in range(len(self.list_char_name)):
            ...
            if self.list_char_name[i].isnumeric():
                ...
                if self.list_char_name[i].isupper() and not self._is_const(name_of_folder):
        ...
        if isScreaming_case:
            ...
            if self.list_char_name[len(self.list_char_name)-1] != '_':
                recommend_new_name += '_'
            recommend_new_name += 'CONST'
    ...
    def _verify_big_letter(self, index):
        ...
        if index > 0:
            if self.list_char_name[index - 1] != '_':
                if self.list_char_name[index - 1].isalpha():
    ...
    @staticmethod
    def _get_chars_from_str(my_str):
