    ...
    def add_proxies(self):
        ...
        while True:
            if self.data5u_order:
                threading.Thread(target=self.add_ip_to_data5u_whitelist, args=(self.data5u_order,))
                while True:
                    try:
    ...
    def add_ip_to_data5u_whitelist(order):
        ...
        while True:
            try:
                ...
                ips = requests.get(f"http://soft.data5u.com/wl/mywhitelist/{order}.html").text.strip(";").split(";")
                if ip not in ips:
                    ips = ";".join(ips[:19]) + f";{ip}"
