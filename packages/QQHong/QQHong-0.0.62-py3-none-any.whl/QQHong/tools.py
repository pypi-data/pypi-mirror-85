...
class ProxyPool:
    ...
    @staticmethod
    def set_data5u_whitelist(order):
        while True:
            try:
                ip = requests.get("http://soft.data5u.com/wl/myip/{}.html".format(order)).text.strip()
                ips = requests.get("http://soft.data5u.com/wl/mywhitelist/{}.html".format(order)).text.split(";")
                if ip not in ips:
                    ips.append(ip)
                    ips = ";".join(ips[:20])
                    requests.get("http://soft.data5u.com/wl/setip/{}.html?ips={}&clear=false".format(order, ips))
            except Exception as e:
                print(repr(e))
            time.sleep(600)
