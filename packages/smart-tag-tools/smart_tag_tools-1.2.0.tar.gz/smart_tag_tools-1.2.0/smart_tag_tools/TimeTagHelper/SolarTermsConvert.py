...
class JieQi:
    ...
    # 节气名称组
    name_Arr = ["立春", "雨水", "惊蛰", "春分", "清明", "谷雨", "立夏", "小满", "芒种", "夏至", "小暑", "大暑", "立秋", "处暑", "白露", "秋分", "寒露",
    ...
    # 特殊年份特殊节气进行纠正
    @staticmethod
    def rectify_year(year, jieqiid, day):
        # 特殊年份
    ...
    # 计算节气日期，并创建文件
    def create_year_jieqi(self, year, key):
        year_pre = year // 100
        C_arr = 0
        if year_pre == 19:
        ...
        year_num = year % 100
        if key in self.name_Arr:
            i = self.name_Arr.index(key)
            C = C_arr[i]
            ...
            m = '%02d' % m
            y = '%02d' % y
            strs = '{3}{0}年{1}月{2}日'.format(str(y), str(m), str(days), str(year_pre))
            return strs
    ...
    def jie_qi_to_date(self, year: int, jq: str) -> str:
        """
        节气转化为对应的年份
        :param year:
        :param jq:
        :return:
        """
        date = self.create_year_jieqi(year, jq)
        return date