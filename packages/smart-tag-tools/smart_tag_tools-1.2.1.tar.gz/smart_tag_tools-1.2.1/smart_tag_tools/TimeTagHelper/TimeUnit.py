...
from smart_tag_tools.TimeTagHelper.RangeTimeEnum import RangeTimeEnum

try:
    from smart_tag_tools.TimeTagHelper.LunarSolarConverter.LunarSolarConverter import *
except:
    from smart_tag_tools.TimeTagHelper.LunarSolarConverter import *
...
class TimeUnit:
    ...
    def time_normalization(self):
        ...
        if self.normalizer.isTimeSpan:
            ...
            if self.tp.tunit[1] > 0:
                days += int(30.417 * self.tp.tunit[1])
    ...
    def norm_setSpanRelated(self):
        ...
        match = pattern.search(self.exp_time)
        if match is not None:
            glo.set_value("is_zhounian", True)
            self.normalizer.isTimeSpan = True
            year = int(match.group())
        ...
        match = pattern.search(self.exp_time)
        if match is not None:
            if not glo.get_value("is_zhounian"):
                self.normalizer.isTimeSpan = True
    ...
    def norm_setHoliday(self):
        rule = "(情人节)|(母亲节)|(父亲节)|(青年节)|(教师节)|(中元节)|(端午)|(劳动节)|(7夕)|(建党节)|(建军节)|(初13)|(初14)|(初15)|" \
               "(初12)|(初11)|(初9)|(初8)|(初7)|(初6)|(初5)|(初4)|(初3)|(初2)|(初1)|(中和节)|(圣诞)|(中秋)|(腊8节)|(春节)|(元宵)|" \
        ...
        if match is not None:
            ...
            elif holi in ["母亲节", "父亲节"]:
                from .TimeNormalizer import TimeNormalizer  # 引入包

                tn = TimeNormalizer()
                if holi == "母亲节":
                    date_temp = tn.parse("周日", str(time.strftime("%Y")) + "-05-01 01:01:01").get("timestamp").split("-")
                    date = [date_temp[1], int(date_temp[2]) + 7]
                elif holi == "父亲节":
                    date_temp = tn.parse("周日", str(time.strftime("%Y")) + "-06-01 01:01:01").get("timestamp").split("-")
                    date = [date_temp[1], int(date_temp[2]) + 14]
            else:
                holi = holi.strip('节')
                if holi in ['小寒', '大寒']:
                    self.tp.tunit[0] += 1
