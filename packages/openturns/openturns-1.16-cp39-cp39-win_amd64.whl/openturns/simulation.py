...
class MartinezSensitivityAlgorithm(SobolIndicesAlgorithmImplementation):
    ...
    **requires an** ``outputDesign`` **produced by** :class:`~openturns.SobolIndicesExperiment`
    **(see example below). Otherwise, results will be worthless.**
    ...
    .. math::
       ...
       \begin{array}{ccc}
       \hat{S}_i & = & \rho_n(\tilde{G}(\mat{B}), \tilde{G}(\mat{E}^i)) \\
       \widehat{ST}_i & = & 1 - \rho_n(\tilde{G}(\mat{A}), \tilde{G}(\mat{E}^i)) \\
       \end{array}
...
class MauntzKucherenkoSensitivityAlgorithm(SobolIndicesAlgorithmImplementation):
    ...
    **(see example below). Otherwise, results will be worthless.**
    ...
    .. math::
       ...
       \begin{array}{ccc}
       \hat{V}_i & = & \frac{1}{N-1} \sum_{k=1}^{N} \tilde{G}(\vect{B}_k) \left( \tilde{G}(\vect{E}_k) - \tilde{G}(\vect{A}_k) \right) \\
       \hat{V}_{-i} & = & \frac{1}{N-1} \sum_{k=1}^{N} \tilde{G}(\vect{A}_k) \left( \tilde{G}(\vect{E}_k) -  \tilde{G}(\vect{A}_k) \right)
       \end{array}
