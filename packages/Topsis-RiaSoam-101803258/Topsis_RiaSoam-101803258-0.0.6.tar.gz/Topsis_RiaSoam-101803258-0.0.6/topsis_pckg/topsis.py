...
def topsis(inputname,Weight,Impact,outputname):
    try:
        ...
        raise Exception("Wrong file / File path error")
    df=pd.read_csv(inputname)
    df2=pd.read_csv(inputname)
    if(len(df.columns)<2):
        raise Exception('there are less than 2 columns')
    df1=df.dtypes
    if(list(df1).count('float64')==df.shape[1]):
        raise Exception('non-numeric values detected')
    weight=[]
    for i in Weight.replace('"','').split(','):
        weight.append(int(i))
    if(len(df.columns[1:])!=len(weight)):
        raise Exception('the length of weight = number of columns')
    impact=[]
    for i in Impact.replace('"','').split(','):
        impact.append(i)
    if(len(df.columns[1:])!=len(weight)):
        raise Exception('the length of impact = number of columns')
    for i in impact:
        if(i!='+' and i!='-'):
