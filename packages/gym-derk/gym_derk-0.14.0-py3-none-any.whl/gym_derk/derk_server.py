...
class DerkSession:
  ...
  Attributes:
    ...
    team_stats: Stats for each team for the last episode. Numpy array of shape (n_teams, len(team_stats_keys))
  ...
  def __init__(self, websocket, init_msg):
    ...
    self.team_stats_keys = enum.Enum('TeamStats', init_msg['teamStatsKeys'], start=0)
    ...
    self.team_stats = np.zeros((self.n_teams, int(len(self.team_stats_keys))))
  ...
  async def step(self, action_n: np.ndarray = None) -> Tuple[np.ndarray, np.ndarray, List[bool], List[Dict]]:
    ...
    if self.done:
      ...
      points = self.team_stats[:, self.team_stats_keys.Points.value]
      opponent_points = self.team_stats[:, self.team_stats_keys.OpponentPoints.value]
  ...
  def _decode_team_stats(self, data):
    ...
    return arr.reshape(int(len(self.team_stats_keys)), -1).transpose()
