...
def template(site: dict):
    for sitename, account in site.items():
        error1 = {}
        loginurl = "https://" + sitename + "/auth/login"
        logouturl = "https://" + sitename + "/user/logout"
        checkinurl = " https://" + sitename + "/user/checkin"
        headers = {
        ...
        for email, passwd in account.items():
            ...
            if "html" not in logout.text or "/html" not in logout.text:
                error1['logout'] = str(logout.status_code)
            s.close()
            if error1:
...
def fpork(account: dict):
    error1 = {}
    loginurl = "https://forever.ypork.com/auth/login"
    logouturl = "https://forever.ypork.com/user/logout"
    checkinurl = "https://forever.ypork.com/user/checkin"
    headers = {
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/85.0.4183.121 Safari/537.36",
    }
    checkinheaders = {
    ...
    for email, passwd in account.items():
        ...
        s.close()
        if error1:
            f = open(r"autocheckin_log.txtst", "a", encoding="utf-8")
...
def bytemplate(isite: dict, iaccount: dict):
    ...
    """
    site = {}
    sitedict = {"1": "pucloud.vip", "2": "forever.ypork.com", "3": "liuns.com"}
    if iaccount:
        for i in isite.keys():
