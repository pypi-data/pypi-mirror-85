...
class TestCMIP6OutputCube(_CMIPCubeIntegrationTester):
    ...
    def test_aermon_co2_plevels(self, test_cube, test_cmip6_output_co2_aermon_file):
        ...
        np.testing.assert_allclose(
            res["World"].cube.data[0, 0], 0.0002876096972782, rtol=1e-6
        )
        np.testing.assert_allclose(
            res["World|Ocean"].cube.data[0, -1], 0.0002864358727, rtol=1e-6
        )
        np.testing.assert_allclose(
            res["World|Land"].cube.data[-1, 0], 0.000289739789488548, rtol=1e-6
        )
        np.testing.assert_allclose(
            res["World|Southern Hemisphere"].cube.data[-1, -1],
            0.00028643743275,
            rtol=1e-6,
        )
        np.testing.assert_allclose(
            res["World|Northern Hemisphere|Ocean"].cube.data[-2, -2],
            0.00028638078818,
            rtol=1e-6,
        )
