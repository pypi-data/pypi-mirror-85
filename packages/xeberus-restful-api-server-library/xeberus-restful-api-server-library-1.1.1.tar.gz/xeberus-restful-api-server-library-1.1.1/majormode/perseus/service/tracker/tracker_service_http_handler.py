...
from majormode.perseus.model.tracker import BatteryStateChangeEvent
from majormode.perseus.model.tracker import LocationUpdate
...
from majormode.perseus.service.tracker.tracker_service import TrackerService

# from majormode.perseus.constant.sort_order import SortOrder
# from majormode.perseus.model import obj
# from majormode.perseus.model.contact import ContactPropertyName
...
# from majormode.tracker.message.device import ErrorReport
...
class TrackerServiceHttpRequestHandler(HttpRequestHandler):
    ...
    def activate_device(self, request, device_id, activation_code):
        ...
        if not request.client_application:
            raise TrackerService.InvalidOperationException("the information of the tracker agent application has not be given (cf. User-Agent header)")
    ...
    @http_request(r'^/tracker/(device_id)/handshake$',
                  ...
                  authentication_required=False,
                  signature_required=True)
    def on_device_handshake(self, request, device_id):
        # Check whether the information of the agent application has been
        # correctly passed.
        if not request.client_application:
            raise TrackerService.InvalidOperationException("the information of the tracker agent application has not be given (cf. User-Agent header)")
        ...
        return TrackerService().on_device_handshake(
            request.app_id,
            device_id,
            seed,
            imei,
