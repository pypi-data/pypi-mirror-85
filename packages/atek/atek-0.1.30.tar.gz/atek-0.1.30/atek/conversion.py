from typing import *
from toolz.curried import pipe, take, map, merge, curry
...
def to_date(pat: str, df: pd.DataFrame) -> pd.DataFrame:
    ...
    dataframe with the word 'date' to a float64 column.
    """
    return df.assign(**{
        col: pd.to_datetime(df[col], errors="coerce")
        for col in df.columns
        if fnmatch(col.lower(), pat)
    })
...
# %%
@curry
def decamel(pats: Union[List[str],str], df: pd.DataFrame) -> pd.DataFrame:
