...
from polyaxon.cli.operations import approve, statuses, logs as run_logs, upload as run_upload
...
def run(
    ...
    def create_run(is_manged: bool = True):
        is_approved = True if upload else None
        try:
    ...
    if upload:
        ctx.invoke(run_upload, path_to=upload_to)
        ctx.invoke(approve)
