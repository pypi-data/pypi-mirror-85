...
class ClassMapping:
    def __init__(self, class_id: UUID, source_mappings: List[Mapping], destination_mappings: List[Mapping]):
        self.class_id = class_id
        self.source_mappings = source_mappings
        self.destination_mappings = destination_mappings

    def __repr__(self):
        return f'{self.__class__.__name__}(class_id: {self.class_id}, source_mappings: {self.source_mappings!r}, ' \
    ...
    def __eq__(self, other):
        if isinstance(other, ClassMapping):
            return (self.class_id == other.class_id and
                    self.source_mappings == other.source_mappings and
                    self.destination_mappings == other.destination_mappings)
        return False
