...
class ConfigTestCase(TestCase):
    ...
    @patch("xray_configurator.xray_configurator.XRayMiddleware")
    @patch("xray_configurator.xray_configurator.XRayFlaskSqlAlchemy")
    def test_xray_defaults(self, mock_alchemy, mock_middleware, mock_patch_all):
        config = configparser.ConfigParser()
        config.read(
        ...
        self.assertEqual(xray_recorder.service, "test-service-name")
        self.assertEqual(xray_recorder.sampling, False)
        self.assertEqual(xray_recorder.stream_sql, True)
    ...
    @patch("xray_configurator.xray_configurator.XRayMiddleware")
    @patch("xray_configurator.xray_configurator.XRayFlaskSqlAlchemy")
    def test_xray(self, mock_alchemy, mock_middleware, mock_patch_all):
        config = configparser.ConfigParser()
        ...
        self.assertTrue(global_sdk_config.sdk_enabled())
        self.assertEqual(xray_recorder.service, "test-service-name")
        self.assertEqual(xray_recorder.sampling, True)
        self.assertEqual(xray_recorder.stream_sql, False)
