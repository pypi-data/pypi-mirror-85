...
class ConfigTestCase(TestCase):
    ...
    def test_xray_defaults(self, mock_alchemy, mock_middleware, mock_patch_all, mock_configure):
        ...
        mock_configure.assert_called_once_with(daemon='xray-daemon:2000', plugins=('ECSPlugin', 'EC2Plugin'), sampling=False, service='test-service-name', stream_sql=True)
    ...
    def test_xray(self, mock_alchemy, mock_middleware, mock_patch_all, mock_configure):
        ...
        XRayConfigurator(config, app_mock)
        self.assertTrue(global_sdk_config.sdk_enabled())
        mock_configure.assert_called_once_with(daemon='129.0.0.0:5000', plugins=('ECSPlugin', 'EC2Plugin', 'ElasticBeanstalkPlugin'), sampling=True, service='test-service-name', stream_sql=False)
