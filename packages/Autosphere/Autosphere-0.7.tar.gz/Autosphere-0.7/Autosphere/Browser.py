...
from Autosphere.core import locators, notebook, webdriver
...
class Browser(SeleniumLibrary):
    """Autosphere Framework library for Browser operations.
    ...
    def _get_driver_args(
        ...
        headless = headless or bool(int(os.getenv("AutoSphere_HEADLESS_MODE", "0")))
        kwargs = {}
    ...
    def _set_user_profile(
        ...
        Requires environment variable ``AutoSphere_CHROME_USER_PROFILE_DIR``
        to point into user profile directory.

        ``options`` dictionary of browser options
        """
        data_dir = profile_path or os.getenv("AutoSphere_CHROME_USER_PROFILE_DIR")
