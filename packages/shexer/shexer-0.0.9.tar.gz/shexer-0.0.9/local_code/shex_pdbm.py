...
# import zipfile
#
# base_source_dir = "C:\\Users\\Dani\\Documents\\EII\\coronavirus\\shex\\misterios_andra\\"
# base_dest_dir = "C:\\Users\\Dani\\Documents\\EII\\coronavirus\\shex\\target\\"
#
# import gzip
# f = gzip.open(base_source_dir + 'CC2rdf.xsl.gz', 'rt')
# file_content = f.read()
# with open(base_dest_dir + 'ahiesta.xsl', "w") as out_str:
#     out_str.write(file_content)
# f.close()
#
#
# print("Done!")
#
#
# def extract_folder_files(gz_source_folder, dest_folder):
#     folder = gzip.open(gz_source_folder, 'rt')
#     for files in folder.na
#
...
def ExtractFolder(folder_name):
    if folder_name:  # if extract_folder_name != None
        # extract_folder_fullpath = os.path.join(os.path.dirname(
        #     folder_name), folder_name)
        WalkTreeAndExtract(folder_name)
        # Given tar file is extracted to extract_folder_name. Now descend
        # down its directory structure and extract all other tar files
        # (recursively).
