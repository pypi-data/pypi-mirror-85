...
class SDM:
    ...
    def sync_sheets2local(self, language: str, test_ratio: float = 0.3):
        ...
        log.debug(current_training_data[0])
        log.debug(train_data[0])
        ...
        log.debug("%s", total_train_data.iloc[0])
        log.debug("%s", total_train_data.iloc[100])
        log.debug("%s", total_train_data.iloc[1000])
