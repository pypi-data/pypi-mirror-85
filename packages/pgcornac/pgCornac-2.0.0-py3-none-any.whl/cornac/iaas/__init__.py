...
class IaaS(object):
    ...
    def connect(cls, url, config):
        ...
        try:
            ...
            # Avoid leaking url in frame.
            del url
            msg = f"Failed to connect to {provider}: {e}"
            raise KnownError(msg) from None
...
def check_url(url):
    try:
        ...
        raise KnownError(f"Failed to contact {url}: {e}")
    return r
