...
def df_to_serializable_dict(series):
    """Transform numpy types that can't be serialized to ones that can"""
    return {
        key: val.item() if isinstance(val, np.generic) else val
        for key, val in series.replace({np.nan: None, np.inf: None}).to_dict().items()
    }
