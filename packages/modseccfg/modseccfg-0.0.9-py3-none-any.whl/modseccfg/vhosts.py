...
class rx:
    ...
    configline = re.compile(
        """ ^
        [\ \\t]*                          # whitespace \h*
        # (?:Use \s{1,4})?                  # optional: `Use␣` to find custom macros like `Use SecRuleDisableByPath…`
    ...
    rule_comment = """
        ...
        id:{}                             # id:nnnnn, requires "…{}…".format($id)
    """
...
class vhost:
    ...
    def __init__(self, fn, src):
        ...
        self.ruledecl = {}     # SecRule IDs → rules{}
        self.linemap = {}      # LineNo → RuleID (for looking up chained rules in error.log)
        self.mk_linemap(src)   # id → source line
        ...
        for dir,args  in rx.configline.findall(src):    # or .finditer()? to record positions right away?
            ...
            #print(dir, args)
            if hasattr(self, dir):
                # src.find(…) + str position to linecounter... / or do a separate line indexer scan (tx.msg:… -> id:…)
