# -*- coding: utf-8 -*-
"""
Created on Fri Nov 13 22:34:37 2020
...
def main():
    ...
    if len(sys.argv) < 5:
        ...
        print('Syntax: python topsis.py file.csv "0.25,0.25,0.25,0.25" "-,+,+,+" res.csv')
        sys.exit()
    ...
    try:
        ...
        print('Error: Invalid format of Weights or Impacts')
        sys.exit()
        
    for each in impacts :
        if each not in ('+','-'):
            print('Error: Invalid format of Impacts')
            sys.exit()
    ...
    if len(list(input_data.columns))<=2:
        print('Error: Input file', file, 'contains less than 3 columns')
        sys.exit()
    ...
    if len(weights) != cols:
        print("Error: Number of weights are sparse")
        sys.exit()
        
    if len(impacts) != cols:
        print("Error: Number of impacts are sparse")
        sys.exit()
