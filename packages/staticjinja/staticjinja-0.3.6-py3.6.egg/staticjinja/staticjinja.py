...
def _make_filter(inp):
    """Make a filter function from either a regex string or an actual function.

    The returned filter function takes a filename as input and returns true or
    false. If a string is passed as input, the string is assumed to be a regex,
    and the result is constructed from that. Otherwise the input is assumed to
    be an already constructed filter, and is just returned as-is.

    :param in:
        Either a regex string or an already built filter function.
    """
    if isinstance(inp, str):
        def filter(filename):
            return bool(re.search(inp, filename))
...
def is_underscored(filename):
    """Returns if a file or any of its parent directories begin with ``'_'``.
...
def is_dotted(filename):
    """Returns if a file or any of its parent directories begin with ``'.'``.
...
class Site(object):
    ...
    :param staticpaths:
        [Deprecated, please use staticfilter]
    ...
    :param templatefilter:
        Used to filter which files are treated as templates. Either a regex to
        match against filenames, or a function that takes a filename and returns
        a boolean. Defaults to ``None``.

    :param staticfilter:
        Used to filter which files are treated as static. Either a regex to
        match against filenames, or a function that takes a filename and returns
        a boolean. Defaults to ``None``.

    :param is_ignored:
        Used to filter which files are treated as ignored. Either a regex to
        match against filenames, or a function that takes a filename and returns
        a boolean. Defaults to a filter that returns if a file or any of its
        parent directories begin with ``'.'``.

    :param is_partial:
        Used to filter which files are treated as partials. Either a regex to
        match against filenames, or a function that takes a filename and returns
        a boolean. Defaults to a filter that returns if a file or any of its
        parent directories begin with ``'_'``.
    """
    ...
    def __init__(self,
                 ...
                 staticpaths=None,
                 mergecontexts=False,
                 is_static=Site.in_staticpaths,
                 is_template=Site.not_static_partial_ignored,
                 is_ignored=is_dotted,
                 is_partial=is_underscored,
                 ):
        ...
        self.rules = rules or []
        if staticpaths:
            warnings.warn("staticpaths are deprecated. Use staticfilter.")
        self.staticpaths = staticpaths or []
        self.mergecontexts = mergecontexts

        self.is_template = _make_filter(is_template)
        self.is_static = _make_filter(is_static)
        self.is_ignored = _make_filter(is_ignored)
        self.is_partial = _make_filter(is_partial)
    ...
    def make_site(cls,
        ...
        :param templatefilter:
            Used to filter which files are treated as templates. Either a regex
            to match against filenames, or a function that takes a filename and
            returns a boolean. Defaults to ``None``.

        :param staticfilter:
            Used to filter which files are treated as static. Either a regex to
            match against filenames, or a function that takes a filename and
            returns a boolean. Defaults to ``None``.

        :param is_ignored:
            Used to filter which files are treated as ignored. Either a regex to
            match against filenames, or a function that takes a filename and
            returns a boolean. Defaults to a filter that returns if a file or
            any of its parent directories begin with ``'.'``.

        :param is_partial:
            Used to filter which files are treated as partials. Either a regex
            to match against filenames, or a function that takes a filename and
            returns a boolean. Defaults to a filter that returns if a file or
            any of its parent directories begin with ``'_'``.
        """
    ...
    def in_staticpaths(self, filename):
        """Check if a file lives in any of the directories specified in
        ``staticpaths``.
    ...
    def not_static_partial_ignored(self, filename):
        """Check if a file is not partial, ignored, or static.
