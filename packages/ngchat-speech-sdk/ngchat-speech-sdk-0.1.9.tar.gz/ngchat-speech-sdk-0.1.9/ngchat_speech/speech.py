...
class SpeechConfig():
    ...
    def __init__(
        ...
        speech_synthesis_voice_name: Text = 'LinTone',
...
class SlothWebsocketParam(object):
    def __init__(self, speech_config: SpeechConfig, token, text, is_ssml):
        ...
        self.settings_args = {
            ...
            "sample_rate": SUPPORTED_AUDIO_FORMAT[speech_config.speech_synthesis_output_format_id]['sample_rate'],
        }
...
class SlothWebsocketClient():
    ...
    def save_wav_file(self, audio_pcm, filename):
        ...
        audio.setframerate(SUPPORTED_AUDIO_FORMAT[speech_config.speech_synthesis_output_format_id]['sample_rate'])
    ...
    def on_message(self, message):
        try:
            ...
            sid = message["sid"]
            if status is not "ok":
                svc_logger.info(f"Sloth websocket: sid: {sid}; status: {status}.")
...
class SpeechSynthesizer(SlothWebsocketClient):
    ...
    def __init__(
        ...
        self.__password = speech_config.password
        if self.get_login_token() is False:
            svc_logger.error(f"Can't get server's url and token for {self.__account_id}, please make sure account_id and password are right.")
            return None
        self.__speech_synthesis_language = speech_config.speech_synthesis_language
        self.__speech_synthesis_voice_name = speech_config.speech_synthesis_voice_name
        self.__output_format = speech_config.output_format
    ...
    def get_login_token(self) -> bool:
        ...
        if response.status_code == 200:
            self.__login_token = response.json()['token']
            return True
        else:
