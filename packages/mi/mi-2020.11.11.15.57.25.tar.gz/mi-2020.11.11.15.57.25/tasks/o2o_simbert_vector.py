...
def texts2vec(texts, is_lite=1, batch_size=1000, maxlen=128):
    X = []
    S = []
    for text in texts:
...
logger.info("下载数据")
os.system(f"{hdfs} -getmerge {in_path}/date={date} data.tsv")

# 转换向量
logger.info("转换向量")
convert_vector_from_file('data.tsv')

# 上传数据: ls -a && rm .data.tsv.crc
logger.info("上传数据")
os.system(f"{hdfs} -mkdir -p {out_path}/date={date}")  # todo: 覆盖
# os.system(f"{hdfs} -put -f data.tsv {out_path}/date={date}/")
os.system(f"{hdfs} -put data.tsv {out_path}/date={date}")
