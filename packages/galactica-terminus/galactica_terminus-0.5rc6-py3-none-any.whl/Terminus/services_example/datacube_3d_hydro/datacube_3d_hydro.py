...
def _convert_center_coord_to_box_size_unit(coord_data, ramses_out_length_unit):
    ...
    if isinstance(coord_data, list):
        if len(coord_data) < 2:
            raise AttributeError("Datacube center coordinate \"{cd!s}\" is not in a valid format".format(cd=str(coord_data)))
        val = coord_data[0]
...
def run(data_path, data_ref, scalar_field='gas density', xcenter=0.5, ycenter=0.5, zcenter=0.5, size=0.1, nxyz=128,
        out_format="FITS"):
    """
    ...
    scalar_field: scalar quantity reference. Available choices are :
        * 'gas density' (default)
        * 'gas temperature'
    ...
    nxyz: datacube resolution (default 128^3)
    ...
    ro.define_amr_scalar_field("hydro", "P", 4)
    ...
    field_list = []
    if scalar_field == 'gas density':
        field_list.append('rho')
    elif scalar_field == 'gas temperature':
        field_list.append('rho')
        field_list.append("P")
    elif re.match("^gas\svelocity\salong\s(x|y|z)\saxis$", scalar_field) is not None:
    ...
    cube_data = p.process(cam, cube_size=size, resolution=nxyz)
    ...
    d = {'service_name': 'datacube_3d_hydro',
         ...
         'run_parameters': {'xcenter': xcenter, 'ycenter': ycenter, 'zcenter': zcenter,
                            'field': scalar_field, 'size': size, 'nxyz': nxyz},
