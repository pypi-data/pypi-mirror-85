...
if __name__ == '__main__':
    dut = junos.JunosDevice("ssh://root:your_password@bj4100a.englab.juniper.net")
    client = linux.LinuxDevice(username = "root", password = "your_password",
            hostname = "ent-vm01.englab.juniper.net",
            interfaces = [ { 'name': 'eth1', 'inet': '41.0.0.2/24', 'inet6': '2001::2/64'} ])
    server = linux.LinuxDevice(username = "root", password = "your_password",
            hostname = "ent-vm02.englab.juniper.net",
            interfaces = [ { 'name': 'eth1', 'inet': '42.0.0.2/24', 'inet6': '2002::2/64'} ])
    ...
    if sessions and int(sessions[0]["flow-information"][0]['pkt-cnt']) > 0 and \
