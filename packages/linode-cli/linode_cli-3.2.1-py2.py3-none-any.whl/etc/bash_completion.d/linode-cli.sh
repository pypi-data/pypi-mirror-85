...
_linode_cli()
{
    ...
    case "${prev}" in
        linode-cli)
            COMPREPLY=( $(compgen -W "account events maintenance users domains default images linodes kernels stackscripts lke longview managed networking firewalls nodebalancers object-storage profile sshkeys regions tickets tags volumes --help" -- ${cur}) )
            return 0
            ;;
        account)
