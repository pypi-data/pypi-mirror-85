...
from fibomat import site, units, pattern, shapes, utils, boundingbox
...
class SpotListBackend(BackendBase):
    ...
    def __init__(
        ...
        if base_dwell_time:
            self._base_dwell_time = units.scale_to(self._time_unit, base_dwell_time)
        else:
    ...
    def process_site(self, new_site: site.Site):
        self._site_pos = units.scale_factor(self._length_unit, new_site.unit) * new_site.center
    ...
    def _rasterize_and_add(self, ptn: pattern.Pattern[shapes.Shape]):
        dwell_points = np.array(ptn.raster_style.rasterize(
            ptn.dim_shape, ptn.shape_mill, self._length_unit, self._time_unit
        ).dwell_points)

        # dwell_points[:, 2] *= units.scale_to(self._time_unit, ptn.shape_mill.dwell_time)
    ...
    def arc_spline(self, ptn: pattern.Pattern[shapes.ArcSpline]) -> None:
        rasterized = ptn.raster_style.rasterize(ptn.dim_shape, ptn.shape_mill, self._length_unit, self._time_unit)
        self._dwell_points.append(
            rasterized.dwell_points
        )

    def rasterized_curve(self, ptn: pattern.Pattern[shapes.RasterizedPoints]):
    ...
    #     self._rasterize_and_add(ptn)
    #
    # def rasterized_curve(self, ptn: pattern.Pattern[rasterizing.RasterizedPoints]):
    #     self._rasterize_and_add(ptn)

    # def rasterized_curve(self, pattern: Pattern):
    #     super().rasterized_curve(pattern)
