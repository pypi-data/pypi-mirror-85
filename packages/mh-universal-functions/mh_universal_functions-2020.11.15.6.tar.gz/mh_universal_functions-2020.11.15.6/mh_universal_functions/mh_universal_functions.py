...
def get_mobile_device_presence(_wifi_devices_dict, _router_ip_address, _router_password,
    ...
    """
    Evaluates whether a bluetooth of WiFi device can be detected.
    ...
    :param: _scan_for_device_type: string, contains the scan type "bluetooth" or "WiFi" or "both"
    :returns: bool, device in dictionaries _wifi_devices_dict or  _bluetooth_devices_dict are present
    """

    def scan_bluetooth():
        print('def scan_bluetooth()')
        if platform == "linux" or platform == "linux2":
            for key in iter(_bluetooth_devices_dict.keys()):
                ...
                if device_present is not None:
                    print(device_present, key, _bluetooth_devices_dict[key])
                    module_logger.info("def 'scan_bluetooth' returns: '" + key + "' and True")
                    return key, True
            module_logger.info("def 'scan_bluetooth returns': None and False")
            return None, False
        else:
            module_logger.info("def 'scan_bluetooth returns': None and False")
            return None, False
        ...
        while index < host_numbers:
            ...
            for key in iter(_wifi_devices_dict.keys()):
                ...
                if (host['NewMACAddress'] == device_present) and (host['NewActive']):
                    module_logger.info("def 'scan_wifi' returns: '" + key + "' and True")
                    return key, True
            index = index + 1
        module_logger.info("def 'scan_wifi returns': None and False")
        return None, False

    def mobile_device_presence():
        if _scan_for_device_type == "bluetooth":
            return scan_bluetooth()
        elif _scan_for_device_type == "wifi":
            return scan_wifi()
        elif _scan_for_device_type == "both":
            bluetooth_key, bluetooth_bool = scan_bluetooth()
            wifi_key, wifi_bool = scan_wifi()

            if bluetooth_key is None and wifi_key is None:
                return None, False
            elif bluetooth_key is not None and wifi_key is not None:
                return bluetooth_key, bluetooth_bool
            elif bluetooth_key is None and wifi_key is not None:
                return wifi_key, wifi_bool
            elif bluetooth_key is not None and wifi_key is None:
                return bluetooth_key, bluetooth_bool
