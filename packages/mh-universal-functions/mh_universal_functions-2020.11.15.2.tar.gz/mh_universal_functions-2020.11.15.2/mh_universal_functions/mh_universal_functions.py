...
# ----------------------------------------------------------------------------------------------------------------------
# bluetooth        external          0.23         0.23           -             print(bluetooth.__version__)
...
def get_mobile_device_presence(_wifi_devices_dict, _router_ip_address, _router_password,
    ...
    def scan_bluetooth():
        if platform == "linux" or platform == "linux2":
            for key in iter(_bluetooth_devices_dict.keys()):
                ...
                print("device_present: ", device_present)
                if device_present is not None:
                    print("def get_bluetooth_device_present_with_pir_on - returned TRUE (" + str(datetime.now()) + ")")
                    return key, True
            print("def get_bluetooth_device_present_with_pir_on - returned FALSE (" + str(datetime.now()) + ")")
            return "", False
        else:
            print("def get_bluetooth_device_present_with_pir_on - returned FALSE (" + str(datetime.now()) + ")")
            return "", False
        ...
        while index < host_numbers:
            ...
            for key in iter(_wifi_devices_dict.keys()):
                ...
                if (host['NewMACAddress'] == device_present) and (host['NewActive']):
                    print("device_present: ", key, device_present)
                    print("def get_wifi_device_present_with_pir_on - returned TRUE (" + str(datetime.now()) + ")")
                    return key, True
            index = index + 1
        print("def get_wifi_device_present_with_pir_on - returned FALSE (" + str(datetime.now()) + ")")
        return "", False
    ...
    def mobile_device_presence():
        if _scan_for_device_type == "bluetooth":
            ...
            return scan_bluetooth() or scan_wifi()
