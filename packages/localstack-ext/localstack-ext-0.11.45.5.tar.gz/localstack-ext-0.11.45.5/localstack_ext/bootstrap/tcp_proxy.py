...
class AttrDict(OmJod):
 def __init__(self,*args,**kwargs):
  OmJoR(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and OmJou(socket,'IPV6_V6ONLY'):
...
class Half(OmJoV):
 def __init__(self,opt,sock,addr,OmJoL):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.OmJoL=OmJoL
  self.name='peer' if self.OmJoL else 'client'
  self.queue=[]
  self.dest=OmJoC
  self.err=OmJoC
  self.ready=OmJoM
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':OmJoM,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':OmJoM,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':OmJoM,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':OmJoM,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
