...
def read_api_key():
 ...
 if key:
  return key
 raise dgeJK('Unable to retrieve API key. Please configure $LOCALSTACK_API_KEY in your environment')
def fetch_key():
 ...
 try:
  ...
  result=requests.post('%s/activate'%config.API_URL,json.dumps(data),verify=dgeJP)
...
def cache_key_locally(api_key,key_b64):
 ...
 timestamp=dgeJI(dgeJp(now_utc()))
 key_raw=to_str(base64.b64decode(key_b64))
 for i in dgeJc(dgeJA(timestamp)):
  key_raw=str_insert(key_raw,i*2,timestamp[i])
 key_b64=to_str(base64.b64encode(to_bytes(key_raw)))
 configs['cached_key']={'timestamp':dgeJp(timestamp),'key_hash':md5(api_key),'key':key_b64}
...
def load_cached_key(api_key):
 ...
 if cached_key.get('key_hash')!=md5(api_key):
  raise dgeJK('Cached key was created for a different API key')
 now=now_utc()
 if(now-cached_key['timestamp'])>MAX_KEY_CACHE_DURATION_SECS:
  raise dgeJK('Cached key expired')
 timestamp=dgeJI(cached_key['timestamp'])
