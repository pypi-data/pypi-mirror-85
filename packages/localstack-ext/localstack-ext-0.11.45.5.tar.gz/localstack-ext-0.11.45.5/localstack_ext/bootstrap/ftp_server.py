...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':DpuVj(perm='m',auth=DpuVc,arg=DpuVc,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=DpuVB)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=DpuVc)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=DpuVc):
...
def main():
 do_start_ftp(DpuVi(sys.argv[1]),asynchronous=DpuVB)
