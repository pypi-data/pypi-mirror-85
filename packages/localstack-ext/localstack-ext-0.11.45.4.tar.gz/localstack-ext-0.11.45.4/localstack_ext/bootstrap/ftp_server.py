...
def apply_patches():
 ...
 extended_proto_cmds.update({'SITE ADDUSER':KBxqY(perm='m',auth=KBxqy,arg=KBxqy,help='Syntax: SITE <SP> ADDUSER USERNAME PASSWORD HOME PRIVS <SP>.')})
...
def start_ftp(port):
 if USE_SUBPROCESS:
  ...
  thread=ShellCommandThread(cmd,outfile=subprocess.PIPE,env_vars=env_vars,quiet=KBxqt)
  thread.start()
  time.sleep(2)
 else:
  thread=do_start_ftp(port,asynchronous=KBxqy)
 time.sleep(2)
 wait_for_port_open(port,retries=10,sleep_time=1.5)
 TMP_THREADS.append(thread)
 return thread
def do_start_ftp(port,asynchronous=KBxqy):
...
def main():
 do_start_ftp(KBxqA(sys.argv[1]),asynchronous=KBxqt)
