...
def install_postgres():
 if not in_docker():
  return
 try:
  run('which postgres',print_error=xLMJB)
 except xLMJS:
  install_alpine_package('postgresql','RDS')
 try:
  run('which pg_config',print_error=xLMJB)
 except xLMJS:
  install_alpine_package('postgresql-dev','RDS')
def install_redis():
 try:
  run('which redis-server',print_error=xLMJB)
 except xLMJS:
  install_alpine_package('redis','ElastiCache')
 return 'redis-server'
def install_mqtt():
 try:
  run('which mosquitto',print_error=xLMJB)
 except xLMJS:
...
def download_github_artifact(url,target_file):
 def do_download(url,print_error=xLMJB):
  try:
