...
class AttrDict(LHBFc):
 def __init__(self,*args,**kwargs):
  LHBFz(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and LHBFQ(socket,'IPV6_V6ONLY'):
...
class Half(LHBFM):
 def __init__(self,opt,sock,addr,LHBFA):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.LHBFA=LHBFA
  self.name='peer' if self.LHBFA else 'client'
  self.queue=[]
  self.dest=LHBFl
  self.err=LHBFl
  self.ready=LHBFe
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':LHBFe,'bind_port':2223,'addr':'192.168.43.191','port':22}
 opt={'ip6':LHBFe,'bind_port':48429,'addr':'128.0.100.3','port':22}
 opt={'ip6':LHBFe,'port':54237,'bind_addr':'128.0.100.2','bind_port':22}
 opt={'ip6':LHBFe,'port':54237,'bind_addr':'192.168.100.3','bind_port':22}
 server_loop(opt)
