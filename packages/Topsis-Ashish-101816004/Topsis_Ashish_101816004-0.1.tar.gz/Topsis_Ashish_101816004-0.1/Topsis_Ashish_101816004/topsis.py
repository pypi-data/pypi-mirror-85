...
log = logging.getLogger("topsis-101816004")

def checkParameter():
	parameters = sys.argv
	if(len(parameters)!=5 ):
		log.error("Please provide all the parameters...\n")
		return 0
	else:
		for filename in sys.argv[1:]:
			if(filename.endswith('.csv')):
				...
				if os.path.exists(File_path) != 1:
					log.error('File with filename %s doesnot exist in root directory!\n' %(filename))
					log.error('PS: Current Directory: ',os.path.abspath(__file__))
					log.error('\nTerminating...')
					return 0
				else:
					fileData = pd.read_csv(filename)
					if len(fileData.columns) <3:
						log.error('File with filename %s contains less than 3 columns\nTerminating...' %(filename))
						return 0
					else:
						...
						for i in range(0,(fileData.shape[0])):
							for j in range(1,len(weights)+1):
								try:
									...
									log.error("File Contains non-numeric values.\n Terminating...")
									return 0
						if (len(sys.argv[2].split(",")) == len(sys.argv[3].split(",")) and len(sys.argv[3].split(",")) == fileData.shape[1]-1):
							...
							for i in impacts:
								if i == "+" or i == "-":
									...
									log.error("Impact must be '+' or '-'\n Terminating...")
									return 0
							for i in weights:
								try:
									i = int(i)
								except:
									log.error("Weights must be integer and separated by a ','.\nTerminating...")
									return 0
							
						else:
							log.error("Incorrect input. Length of impact,weights and number of columns(except 1) must be equal. \n Terminating...")
			else:
				log.error('File with filename : %s is not supported(must end with *.csv )\nTerminating...' %(filename))
				return 0
			return 1
