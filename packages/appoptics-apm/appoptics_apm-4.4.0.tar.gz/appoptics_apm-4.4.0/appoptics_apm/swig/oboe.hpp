...
public:
    ...
    Reporter(
             ...
             std::string proxy               // HTTP proxy address and port to be used for the gRPC connection
             ) {

        oboe_init_options_t options;
        memset(&options, 0, sizeof(options));
        options.version = 9;
        int result = oboe_init_options_set_defaults(&options);
        if (result != OBOE_INIT_OPTIONS_SET_DEFAULTS_OK) {
            std::cerr << "Could not set defaults. Error code is " << result << std::endl;
        }
