...
def getNameCity(nCity):
    ...
    if(nCity==7): return "Zurich"  
    elif(nCity==6): return "Locarno"
    else:
        return "Unknown"
...
def getDayIndexByHourOfYear(_hour):
    ...
    for myYear in range(30,0,-1):
        if(_hour>myYear*8760.):
            year=myYear            
            break;

    hour = _hour - 8760.*year            
...
def getNumberOfHoursPerMonth(_month):
    ...
    return (getNumberOfDaysOfMonth(_month)*24.)   
...
the values are stored in varMonth as varMonth[0]=sum of Jan, etc..
"""       
def calculateMonthlyValuesMinMaxAvgFromUserDefinedTimeStep(varUserDefined,timeStepInSeconds,firstHourInYear=0,avoidZeros=False):
...
def getTLog(tIn,tOut,tStorage):
        ...
        if (dtIn<=0 or dtOut<=0 or dtIn*dtOut<0. or dtIn-dtOut<1e-10):
            dtlm=0.             
        else:
...
def calcQvsT(tFlow,eFlow,nameFile=None):              
    ...
    if(nameFile != None):
        ...
        for i in range(numberOfTimesteps):            
            line = "%f %f %f\n" % (i+1,tFlowSort[i],cumulativeEFlow[i]);lines = lines+line
...
#        if(varSort[i]<1e-10): varSort[i]=varSort[i-1]
    ...
    if(nameFile != None):
        ...
        for i in range(numberOfTimesteps):            
            line = "%f %f %f\n" % (i+1,tFlowSort[i],varSort[i]);lines = lines+line
...
def calcAndPrintQVersusT(outputPath,fileName,tShFl,tShRt,eSh,tColFl,eColToTes,eColToIce,eColToHp,tHpCond,eHpCond,tIceOut,eIceToTes,tHpEvap,eHpEvap,printEvery=1,qTesFromColUsed=False):              
    ...
    for i in range(numberOfTimesteps):        
        if(i!= 0 and i!=numberOfTimesteps-1):
            if(i%printEvery==0):        
                line = "%d %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n" % (i+1,tShFlSort[i],cumulativeEShFl[i],tShRtSort[i],cumulativeEShRt[i],tColFlSort[i],\
                cumulativeEColToTes[i],cumulativeEColToIce[i],cumulativeEColToHp[i],tHpCondSort[i],cumulativeEHpCond[i],tIceFlSort[i],\
                cumulativeEIceToTes[i],tHpEvapSort[i],cumulativeEHpEvap[i]); lines = lines+line
...
def calcAndPrintQVersusT_new(outputPath,fileName,tShFl,tShRt,eSh,tColFl,eColToTes,eColToIce,eColToHp,tHpCond,eHpCond,tIceOut,eIceToTes,tHpEvap,eHpEvap,eIceToHp,eIceToHpSeries,printEvery=1,qTesFromColUsed=False):              
    ...
    for i in range(numberOfTimesteps):        
        if(i!= 0 and i!=numberOfTimesteps-1):
            if(i%printEvery==0):        
                ...
                cumulativeEColToTes[i],cumulativeEColToIce[i],cumulativeEColToHp[i],tHpCondSort[i],cumulativeEHpCond[i],tIceFlSort[i],\
                cumulativeEIceToTes[i],tHpEvapSort[i],cumulativeEHpEvap[i],cumulativeEIceToHp[i],cumulativeEIceToHpSeries[i]); lines = lines+line
