...
#        self.numberOfDataPoints = len(lines)-1
        ...
        if(typeVariables=="float"):
            ...
            self.variables = np.arange(self.numberOfVariables*self.numberOfDataPoints,dtype=object).reshape(self.numberOfVariables,self.numberOfDataPoints)
...
#                        [self.numberOfVariables][self.numberOfDataPoints]
                        ...
                        try:
                            self.variables[j][i - startIndex] = time.mktime(datetime.datetime.strptime(dateExt+splittedLine[j], fmt).timetuple())
        ...
        for i in range(self.numberOfDataPoints):
            ...
            for j in range(self.numberOfVariables):
                try:                   
                    ...
                    try:
                        self.variables[j][i] = time.mktime(datetime.datetime.strptime(splittedLine[j], "%Y.%m.%d %H:%M:%S.%f").timetuple())
                    except:
