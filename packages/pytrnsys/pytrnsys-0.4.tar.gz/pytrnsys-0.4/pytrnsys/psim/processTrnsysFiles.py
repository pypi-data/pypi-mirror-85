...
#    def readTrnsysDeck(self):
    ...
    def loadFiles(self):
        ...
        self.loader = SimulationLoader(self.outputPath + '//temp', fileNameList=self.fileNameListToRead, mode=self.loadMode,monthlyUsed=self.monthlyUsed,hourlyUsed=self.hourlyUsed,timeStepUsed=self.timeStepUsed)
    ...
    def addDemands(self):
        ...
        self.doc.addTableMonthlyDf(var,legend, "kWh", caption, nameFile,self.myShortMonths,sizeBox=15,addLines=addLines)
    ...
    def addSPFSystem(self,printData=False):
        ...
        self.doc.addTableMonthlyDf(var,legend,["","kWh","kWh","-"],caption, nameFile,self.myShortMonths,sizeBox=15)

        yearlyFactor=10.

        namePdf = self.plot.plotMonthlyDf(self.SPFSHP,"$SPF_{SHP}$",nameFile,yearlyFactor,self.myShortMonths,myTitle=None,printData=printData)
    ...
    def addHeatBalance(self,printData=False):
        ...
        namePdf = self.plot.plotMonthlyBalanceDf(inVar, outVar, niceLegend, "Energy Flows", nameFile, "kWh",self.myShortMonths,yearlyFactor=10,
        ...
        line = "$Q_D$ & %.2f & MWh \\\\ \n"%(totalDemand/1000.); addLines = addLines+line
        line = "Imb & %.1f & %s \\\\ \n"%(100*imb/totalDemand,symbol); addLines = addLines+line

        self.doc.addTableMonthlyDf(var, names, "kWh", caption, nameFile,self.myShortMonths,sizeBox=15,addLines=addLines)
        self.doc.addPlot(namePdf, caption, nameFile, 12)
    ...
    def addElConsumption(self, printData=False):
        ...
        namePdf = self.plot.plotMonthlyBalanceDf(inVar, outVar, niceLegend, "El heat system", nameFile, "kWh",
