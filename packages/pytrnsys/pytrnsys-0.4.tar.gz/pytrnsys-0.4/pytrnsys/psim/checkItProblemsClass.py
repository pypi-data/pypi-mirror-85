...
class checkIterationProblemsClass(processTrnsys.ProcessTrnsysBase):
    ...
    def loadStatus(self,_name):
        ...
        if (year == False):
            self.readTrnsysFiles.readUserDefinedFiles(_name, firstConsideredTime=firstConsideredTime,
                                                      useOnlyOneYear=useOnlyOneYear)
        else:
...
#the whole case only for the it Problems
    ...
    def writeItFileWithSatus(self,name,limit=100):
        ...
        lines=""
        line="Number of iteration problems %f\n "%len(self.hourWhereItProblems);lines=lines+line
            ...
            if(count>limit):
                ...
                for i in range(self.nData):
                    ...
                    if(abs(self.variables[0][i]-time)<self.error): #error is the time step /10.
                        count = count + 1
                        line = "========================================\n";lines=lines+line
                        line = "Iteration problem at :%f\nInvolved Types :" % time;lines=lines+line
                        for m in range(len(self.namesUnitsItProblems[t])):
                            line = "%s:%s\t"%(self.unitsItProblems[t][m],self.namesUnitsItProblems[t][m]);lines=lines+line
                        line="\n";lines=lines+line
                        line = "========================================\n";lines=lines+line

                        for j in range(i-1,i+1,1):
                            for k in range(len(self.variables)):
                                line ="%s=%8.3f\t"%(self.nameVariables[k],self.variables[k][j]);lines=lines+line
                            line="\n";lines=lines+line
