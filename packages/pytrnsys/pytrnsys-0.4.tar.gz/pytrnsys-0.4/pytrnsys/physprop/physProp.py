...
#                print rho
    ...
    def getCp(self,T):
        ...
        if(self.useConstantProperties):
            ...
            try:
                if(newVersion==True): return (PropsSI('C','T',T+273.15,'P',self.pressure,self.name))
                else: return (Props('C','T',T+273.15,'P',self.pressure,self.name)*1000.)
                ...
                if(newVersion==True):
                    ...
                    try:
                        return (Props('C','T',T+273.15,'P',self.pressure,self.name)*1000.)
                    except: 
                        return (Props('C','T',T+0.01+273.15,'P',self.pressure,self.name)*1000.)
    ...
    def getEnthalpy(self,T):
        ...
        if(self.useConstantProperties):
            ...
            if(newVersion==True): return (PropsSI('H','T',T+273.15,'P',self.pressure,self.name)*1000.)
            else: return (Props('H','T',T+273.15,'P',self.pressure,self.name)*1000.)
#            return (HAPropsSI('H','T',298.15,'P',101325,'R',0.5))
            
    def getTemperature(self,h):

        if(newVersion==True): return (PropsSI('T','H',h/1000.,'P',self.pressure,self.name)-273.15)
        else: return (Props('T','H',h/1000.,'P',self.pressure,self.name)-273.15)
    ...
    def getSaturationProperties(self,propertie,valueAtSat):
        ...
        if(propertie=="T"):            
            return (PropsSI('T','P',valueAtSat/1000.,'Q',0,self.name)-273.15) #temperature in K
            
        # Pressure at saturation from temperature
            
        elif(propertie=="P"):
            return 1000.*PropsSI('P','T',valueAtSat+273.15,'Q',0,self.name) #Pressure in KPa = Pa
           
            
        # Enthalpy at saturation from temperature    
            
        elif(propertie=="HLiquidFromT"):
            return 1000.*PropsSI('H','T',valueAtSat+273.15,'Q',0,self.name) #Temperature in K
        elif(propertie=="HLiquidFromP"):
            return 1000.*PropsSI('H','P',valueAtSat/1000.,'Q',0,self.name) #pressure in kPa
        elif(propertie=="HVaporFromT"):
            return 1000.*PropsSI('H','T',valueAtSat+273.15,'Q',1,self.name) #Temperature in K
        elif(propertie=="HVaporFromP"):
            return 1000.*PropsSI('H','P',valueAtSat/1000.,'Q',1,self.name) #pressure in kPa
        ...
        # Density at saturation from temperature    
            ...
            return PropsSI('D','P',valueAtSat/1000.,'Q',0,self.name) #pressure in kPa
        elif(propertie=="DVaporFromT"):
            return PropsSI('D','T',valueAtSat+273.15,'Q',1,self.name) #Temperature in K
        elif(propertie=="DVaporFromP"):
            return PropsSI('D','P',valueAtSat/1000.,'Q',1,self.name) #pressure in kPa
        else:
