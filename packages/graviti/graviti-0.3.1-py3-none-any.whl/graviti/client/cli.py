...
from .contentset import ContentSegmentClient, FusionContentSegmentClient
from .gas import GAS
...
def create(obj: Dict[str, str], name: str) -> None:
    ...
    if info.type != TBRNType.DATASET:
        ...
        sys.exit(1)
    _gas(**obj).create_contentset(info.dataset_name)
...
def publish(obj: Dict[str, str], name: str) -> None:
    ...
    if info.type != TBRNType.DATASET:
        click.echo(f'"{name}" is not a dataset', err=True)
        sys.exit(1)
    dataset = _gas(**obj).get_contentset(info.dataset_name)
    dataset.publish()
...
def delete(obj: Dict[str, str], name: str, yes: bool) -> None:
    ...
    _gas(**obj).delete_contentset(info.dataset_name)
...
def cp(  # pylint: disable=invalid-name, too-many-arguments
    ...
    if info.type in [TBRNType.SEGMENT, TBRNType.NORMAL_FILE]:
        ...
        dataset = _gas(**obj).get_or_create_contentset(info.dataset_name)
...
def _echo_segment(
    dataset_name: str,
    segment_name: str,
    segment: Union[ContentSegmentClient, FusionContentSegmentClient],
    list_all_files: bool,
) -> None:
    """Echo a segment.
    ...
    """
    if isinstance(segment, ContentSegmentClient):
...
def _ls_dataset(gas: GAS, info: TBRN, list_all_files: bool) -> None:
    dataset = gas._get_contentset(info.dataset_name)  # pylint: disable=protected-access
...
def _ls_frame(gas: GAS, info: TBRN, list_all_files: bool) -> None:
    dataset = gas.get_fusion_contentset(info.dataset_name)
...
def _ls_normal_file(  # pylint: disable=unused-argument
    ...
    dataset = gas.get_contentset(info.dataset_name)
...
def ls(  # pylint: disable=invalid-name
    ...
    if not tbrn:
        for dataset_name in gas.list_contentsets():
