...
from .Events import (EventInstance, EventClass, get_recorded_events, Tap, LongTap, Swipe, Text,
        EnterKey, PowerButton)
...
def _get_swipe_direction(dx: int, dy: int) -> str:
    """
    ...
    return list(_SWIPE_DIRECTION_TO_ANGLE.keys())[index]

def _get_swipe_vector(direction: str, length: int) -> Tuple[int, int]:
    """
    Given a direction value and length, return a corresponding vector (x, y).
    """

    angle = _SWIPE_DIRECTION_TO_ANGLE[direction] + uniform(-pi / 8, pi / 8)
...
def _random_swipe(bounds: Tuple[Tuple[int, int], Tuple[int, int]], direction: Optional[str],
    ...
    if direction is None or direction == "":
        direction = choice(list(_SWIPE_DIRECTION_TO_ANGLE.keys()))
    ...
    dx, dy = _get_swipe_vector(direction, length)

    # create event
    return Swipe(x, y, dx, dy, _DEFAULT_SWIPE_DURATION)

def _random_within_bounds(bounds: Tuple[Tuple[int, int], Tuple[int, int]]) -> Tuple[int, int]:
    """
...
class EventTuple:
    ...
    def print_pretty(self, padding = "") -> str:
        s = padding + _EVENT_TYPE_TO_STR[self.action] + " during " + self.activity + "\n"
    ...
    def __str__(self) -> str:
        s = self.activity + "\\" + _EVENT_TYPE_TO_STR[self.action] + "\\"
        for attr in ("direction", "component_id", "component_class"):
            value = getattr(self, attr, "")
            if value != "":
                s += value
    ...
    def create_from_string(string: str) -> Optional["EventTuple"]:
        ...
        if len(fields) != 5 or fields[1] not in _EVENT_STR_TO_TYPE:
            return None

        event_tuple = EventTuple(fields[0], _EVENT_STR_TO_TYPE[fields[1]])
...
class MonkeyLabStrategy(TestingStrategy):
    ...
    def _determine_event_tuple(self, event: EventInstance, use_cache: bool) -> Optional[EventTuple]:
        ...
        if (event_type in (Tap, LongTap, Swipe) and
                self.dc.get_window_at(self.device, event.x, event.y) == "Keyboard"):
            event_type = Text
        ...
        if event_type in (Tap, LongTap, Swipe):
            ...
            if component is None:
                ...
                if event_type == Swipe:
                    event_tuple.direction = _get_swipe_direction(event.dx, event.dy)
    ...
    def _mine_event_tuples(self,
        ...
        TIME_STANDARDIZATION_DELTA = int(time() * 1000) - recorded_events[0][0]
        ...
        for event_info in recorded_events:
            ...
            current_time = int(time() * 1000) - TIME_STANDARDIZATION_DELTA
        ...
        if event_tuple.action in (Tap, LongTap, Swipe):
            ...
            if component is not None:
                if event_tuple.action == Tap:
                    event = Tap(*_random_within_bounds(component.bounds))
                elif event_tuple.action == LongTap:
                    event = LongTap(*_random_within_bounds(component.bounds),
                            _DEFAULT_LONG_TAP_DURATION)
                else:
                    event = _random_swipe(component.bounds, event_tuple.direction,
    ...
    def _generate_random_event(self, gui: Optional[GuiComponent]) -> EventInstance:
        """
        Generate a random Tap, LongTap, or Swipe event.
        """

        # choose a random event type and get all corresponding GUI components
        event_type = choice((Tap, LongTap, Swipe))
        components = self.dc.search_gui_components(gui, event_type = event_type, enabled = True)

        # If components were found, originate the event at a random found component; if no
        # components were found, then choose a random part of the root component. If the root
        # component was not able to be found, then choose a random part of the top 80% of the
        # screen, as to avoid hitting the home button
        if len(components) > 0:
            bounds = choice(components).bounds
        elif gui is not None:
            bounds = gui.bounds
        else:
            bounds = ((0, 0), (self.device.screen_width, int(0.8 * self.device.screen_height)))
        
        # create the event, randomizing other event attributes as necessary
        if event_type == Tap:
            event = Tap(*_random_within_bounds(bounds))
        elif event_type == LongTap:
            event = LongTap(*_random_within_bounds(bounds), _DEFAULT_LONG_TAP_DURATION)
        else:
            event = _random_swipe(bounds, None, self.device.screen_width, self.device.screen_height)
        ...
        while event_count < num_events:
            ...
            if validation_successful:
                ...
                if repeated_failures >= FAILURES_BEFORE_RANDOM:
                    event = self._generate_random_event(self.__cached_gui)
                    if self.__debug:
    ...
    def execute(self, num_events: int, perform_recording: bool = True,
        ...
        """
        Executes the MonkeyLab Strategy and returns the success result (True if the test was
        ...
        num_events: The number of events to execute.

        perform_recording: Whether or not to record and mine events before the language model is
        created. If set to False, then data is pulled from past MonkeyLab sessions and the 
        ...
        if automatic_app_restart:
            self.restart_app()
            input("[GENERATE+VALIDATE] Make sure the app is at the starting screen, then press " +
                    "Enter.")
        else:
