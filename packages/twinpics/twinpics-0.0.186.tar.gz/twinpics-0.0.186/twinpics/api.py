...
def graph_json_values(df_graph, df_basic_data, node_list_df, from_verified):
    ...
    return json.dumps(data), node_list_verified_filter_df
...
def nodes_dict_participants(participants_df):
    l=[]
...
def messages_connections (msg, node_dict_df):
    ...
    for i in range(len(graph_rel)):        
        try:
            s.append([node_dict[graph_rel.screen_name.iloc[i]],node_dict[graph_rel.screen_name_mention.iloc[i]]])
        except KeyError:
             print(graph_rel.screen_name_mention.iloc[i], 'this ID has left the group!')
    df_graph = pd.DataFrame(data=s,columns = ['screen_name','screen_name_mention'])
    df_graph ['iteration'] = [1 for i in range(len(df_graph))]
    return df_graph


def df_graph_telegram_features (DIG, nodes):
    DiG.remove_nodes_from(['_id','name'])
    DiG.add_nodes_from(nodes['name'])
