...
def contextual_analysis_telegram(messages, corpus):
    messages = messages_preprocessing(messages)
...
def sentimental_analysis_telegram(data):
    ...
    tweets_df = sentimental_analysis_features_telegram(data)
    tweets_df["terrorist_suggestion"] = 0
    tweets_df["terrorist_suggestion"][(tweets_df["terrorist_content_similarity"]>3) | (tweets_df["hashtags_terrorist_similarity"]>0)] = 1
    tweets_irr_words = len(tweets_df[tweets_df["terrorist_suggestion"]==1])
    if len(tweets_df[tweets_df["terrorist_suggestion"]==1]) == 0:
      data["per_hash_key"] = 0
      data["negative_opinion"] = 0
