...
class SofaScore:
  ...
  def nations():
    url = "https://api.sofascore.com/api/v1/sport/football/2020-11-15/19800/categories"
    ...
    res = expand_df(pd.DataFrame(res['categories']))[['category>name','category>id','uniqueTournamentIds']]
  ...
  def tournament_seasons(tournament_id=17):
    ...
    res = json.loads(response.text.encode('utf8'))
    res = ep.expand_df(pd.DataFrame(res))
    return res
