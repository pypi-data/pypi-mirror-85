...
class WatsonMachineLearningEngine:
    ...
    def _delete_models(self, model_name):
        ...
        for model in (models['resources'] + functions['resources']):
            if self._is_v4 and not self._is_icp:
                    ...
                    for deployment in deployments['resources']:
                        ...
                        if self._is_v4:
                            if not self._is_icp:
                                ...
                                href = deployment['entity']['asset']['href']
                                model_href = href.split('/v4/models/')
                                function_href = href.split('/v4/functions/')
                                if len(model_href) > 1:
                                    href_split = model_href
                                elif len(function_href) > 1:
                                    href_split = function_href
                                deployment_asset_id = href_split[1].split('?space_id')[0]
                        else:
