...
class OpenScaleClient(OpenScaleReset):
    ...
    def _generate_one_explain(self, scoring_id, background_mode):
        ...
        while status == 'in_progress':
            logger.log_info("Waiting for explanation generation to finish, current status: {}, re-check in 15 seconds ...".format(status))
            time.sleep(15)
            total_time += 15
    ...
    def _get_available_scores(self, max_explain_candidates):
        try:
            ...
            scoring_ids = [payload_table["records"][0]["entity"]["values"]["scoring_id"]]
    ...
    def generate_explain_requests(self):
        ...
        try:
            ...
            explain_ids = self._client.monitor_instances.explanation_tasks(
            ...
            for i in range(num_explains):
                if pause > 0.0 and i > 0:
                    time.sleep(pause)
                scoring_id = explain_ids['metadata']['explanation_task_ids'][i]
