...
class PythonSyntaxHighlighter(qt.QSyntaxHighlighter):
    def __init__(self, parent=None):
        ...
        self.rules = [(qt.QRegExp(r"\b%s\b" % kwd), self.keywordFormat)
                      for kwd in self.keywords] + \
                     [(qt.QRegExp(r"\bdef\s+([A-Za-z_]+[A-Za-z0-9_]+)\s*\("),
                       self.defFormat),
                      (qt.QRegExp(r"\bclass\s+([A-Za-z_]+[A-Za-z0-9_]+)\s*\("),
                       self.defFormat),
...
class OWPythonScript(widget.OWWidget):
    ...
    class Inputs:
        data = Input("data", TomwerScanBase, replaces=["in_data"],
    ...
    def __init__(self):
        ...
        self.libraryView = qt.QListView(
            editTriggers=qt.QListView.DoubleClicked |
            qt.QListView.EditKeyPressed,
            sizePolicy=qt.QSizePolicy(qt.QSizePolicy.Ignored,
