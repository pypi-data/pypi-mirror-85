...
class Tokenizer(tokenization_abc.Tokenization):
    ...
    def ngrams2tokens(self, n: Union[int, Iterable[int]], docs_in:Iterable[Dict[str, str]], docs_out: Iterable[Dict[str, str]]=None, pos: Tuple[str, ...] = ("PROPN", "VERB", "NOUN", "ADJ"),
        ...
        Args:
            docs_in (Iterable[Dict[str, str]]): Stream of input json documents as dicts. <br/> Each document must have a key where the text of the document is stored. Frequent ngrams will be calculated by scrolling this stream.
            docs_out (Iterable[Dict[str, str]], optional, default=None): Stream of output json documents as dicts. <br/> Each document must have a key where the text of the document is stored. If not None, frequent ngrams will be considered as unique tokens for each document in the stream. Each document will be yielded as output of this function.
    ...
    def run(self, docs:Iterable[Dict[str, str]], doc_id: int = 0, token_id: int = 0, sent_id: int = 0, para_id: int = 0,
