...
class GlobalCalling:
    ...
    def log_query(*args):
        ...
        if len([*args]) == 1:
            ...
            query_list.append({"connection": query_dict["connection"], "query": query_dict["query"],
                               "time": query_dict["time"], "success": query_dict["success"]})
        else:
            try:
                ...
                GlobalCalling.log_query({"connection": connection, "query": query, "time": total_sql_time, "success": True})
                if GlobalCalling.looq.test_mode:
                ...
                total_sql_time = datetime.datetime.now() - start_time
                GlobalCalling.log_query({"connection": connection, "query": query, "time": total_sql_time, "success": False})
                if GlobalCalling.looq.test_mode:
                    print("Connection: " + connection + "\nQuery: " + query + "\nTime: " + total_sql_time + "\nSuccess: False")
