...
def main():
    ...
    if len(weights)<2 or len(impact)<2:
        print("Invalid. Weights and impact should be separated by ',' or be more than 2.")
        exit()


    if(len(weights) != len(impact)):
        print("Invalid. Lengths of weight and impact are not equal.")
        exit()

    k = 0
    for i in impact:
        if i not in ["+", "-"]:
            print("Invalid character found. Use only '+' and '-'.")
            exit()
    ...
    if len(new_df) < 2:
        print("Invalid.Columns not correct.")
        exit()
    ...
    out = sys.argv[4]
    res.to_csv("./{}.csv".format(out), index = False)
