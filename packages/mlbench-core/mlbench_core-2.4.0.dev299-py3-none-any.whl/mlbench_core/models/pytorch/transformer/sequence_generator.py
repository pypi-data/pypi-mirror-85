        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            if step < maxlen:
                if prefix_tokens is not None and step < prefix_tokens.size(1):
                    ...
                    torch.floor_divide(cand_indices, self.vocab_size, out=cand_beams.resize_(0))
            ...
            if step >= self.minlen:
                ...
                print(cand_bbsz_idx[:, :beam_size].shape, eos_mask[:, :beam_size].shape, eos_bbsz_idx.shape)
                torch.masked_select(
