...
async def init_db(
    ...
    # TODO AWARE-489 remove as it is deleting the cache
    try:
        path.unlink()
    except:
        pass
...
class MqttStorage:
    ...
    async def insert(self, topic: str, payload: str, qos: int, uid: str):
        """
        ...
        for _ in range(2):
            try:
                await self.conn.execute(
                    "INSERT INTO mqtt (topic, payload, qos, uid) values (?,?,?,?)",
                    (topic, payload, qos, uid),
                )
                return
    ...
    async def _insert_delayed(
        self, topic: str, payload: str, qos: int, uid: str, delay: float
    ):
        await asyncio.sleep(delay)
        return await self.insert(topic, payload, qos, uid)

    def insert_delayed(
        self, topic: str, payload: str, qos: int, uid: str, delay: float = 2
    ) -> asyncio.Future:
        assert qos in range(3)
        return asyncio.create_task(
            self._insert_delayed(topic, payload, qos, uid, delay)
        )
