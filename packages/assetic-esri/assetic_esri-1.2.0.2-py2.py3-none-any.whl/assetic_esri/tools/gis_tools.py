...
class GISTools(object):
    ...
    def get_nm_label_default_dicts(self, layer_name):
        ...
        for c in cps:
            cp_type = None
            if 'component_type' in c['defaults']:
                cp_type = c['defaults']['component_type']
            cp_label = None
            if 'label' in c['defaults']:
                cp_label = c['defaults']['label']
            dims = c['dimensions']
            for d in dims:
                defs = d['defaults']  # type: dict
                defs.update({'component_type': cp_type})
                defs.update({'label': cp_label})
        ...
        for nmdict in xml_val_list:
            ...
            for xmlval, defval in nmdict.items():
                odata_val = None
                if xmlval in cust_config_xml_defs.nm_fields_dict:
                    odata_val = cust_config_xml_defs.nm_fields_dict[xmlval]
                elif xmlval in cust_config_xml_defs.component_fields_dict:
                    odata_val = cust_config_xml_defs.component_fields_dict[
                        xmlval]
                if not odata_val:
                    self.logger.warning(
                        "Unable to find definition for network measure "
                        "configuration '{0}'".format(xmlval))
                    continue
                try:
