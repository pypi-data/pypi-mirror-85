...
def get_platform_core(device,
    ...
    if device.is_ha:
        ...
        dir.append('stby-{dd}'.format(dd=default_dir))
    ...
    for storage in dir:
        ...
        if storage[-1] != ':':
            storage += ':'
        cmd = "dir {s}/core/".format(s=storage)
        ...
        for corefile in corefiles:
            ...
            local_path = "{lp}core/{fn}".format(lp=storage, fn=corefile)
            ...
            if decode:
                ...
                if delete_core:
                    try:
                        device.execute(
                            'delete /force {s}core/*core*.gz'.format(
                                s=storage))
                    except Exception as e:
                        raise Exception(
                            'deleting core files failed. {}'.format(e))
