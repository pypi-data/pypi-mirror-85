...
class CozytouchHeater(CozytouchDevice):
    ...
    async def async_turn_on(self):
        ...
        if self.widget == DeviceType.PILOT_WIRE_INTERFACE:
            await self.async_set_targeting_heating_level(TargetingHeatingLevelState.COMFORT)
        elif self.widget == DeviceType.HEATER:
            await self.async_set_operating_mode(OperatingModeState.INTERNAL)

    async def async_turn_off(self):
        from .constant import DeviceType, OperatingModeState, TargetingHeatingLevelState
        if self.widget == DeviceType.PILOT_WIRE_INTERFACE:
            await self.async_set_targeting_heating_level(TargetingHeatingLevelState.OFF)
        elif self.widget == DeviceType.HEATER:
            await self.async_set_operating_mode(OperatingModeState.STANDBY)
