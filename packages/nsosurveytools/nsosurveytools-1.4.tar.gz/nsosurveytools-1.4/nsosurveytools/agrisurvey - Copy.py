...
def selectplots(plotsDF, args=''):
    ...
    if int(stmgrp) <=104 :
        ...
        PRINTUpdateStatus('\n\n\n     Random Column Out of Range : '+str(stmgrp)+'\n\n')
        sys.exit()
...
def selectplotsFile(FILENAME, args=''):
    global plotsDF, startRow, FileFullPath
    FileFullPath=os.path.abspath(FILENAME).replace(FILENAME,'')
...
def agriGUI():
    ...
    window.geometry("440x360") # Width x Hieght
    ...
    programInfo = { '1': 'Finds & Adds the Missing Plots.' ,
                    '2': 'Does the Plots Selection ',
                    '3': 'Gives All the Possible Subplots in Sequence' }

    
    def RunProg():
        global singleVar, name_entry, stratumVar, osVar, Txtbox
        ...
        windowSize = "550x650" if int(radVal.get()) < 3 else "400x550"
        ...
        if int(radVal.get()) < 3 :
            ...
            if mbx: 
                ...
                agriplot(File, checkfile=checkfile, cwindow=cwindow, args=stratumVar.get()+osVar.get())            
                ...
                if InputFileData.isdigit():
                    ...
                    selectplotsFile(File, args=stratumVar.get()+osVar.get())
    ...
    def enterData():
        ...
        if int(radVal.get()) < 3:
            mainlabel = 'Excel File: ' if int(radVal.get()) == 1 else 'ExcelFile or Total Plots: '
        ...
        if int(radVal.get()) < 3 :
            browsebutton =  tk.Button(inputFrame, text="Browse", command=browsefile)
    ...
    statusLabel = tk.Label(window, text = programInfo[radVal.get()])
