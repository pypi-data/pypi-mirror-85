...
def getListOfTables():
    ...
    tables =  [['arinvoice',['_orgid', 'businesspartner_id', 'deleted', 'docissue', 'invoice_id', 'transactiontype']],
               ['arinvoiceinstallment',['_orgid', 'invoice_id', 'invoiceinstallment_id', 'duedatecash', 'deleted']],
               ['arinvoicepayments', ['_orgid', 'amount', 'amountlocal', 'currency_id', 'deleted',
                                      'invoiceinstallment_id', 'invoicepayments_id', 'issuedate',
                                      'paymentdate', 'paymentstype_id', 'paymentsreference']],
               ['apinvoice', ['_orgid', 'businesspartner_id', 'deleted', 'docissue', 'invoice_id', 'transactiontype']],
               ['apinvoiceinstallment', ['_orgid', 'invoice_id', 'invoiceinstallment_id', 'duedatecash', 'deleted']],
               ['apinvoicepayments', ['_orgid', 'amount', 'amountlocal', 'currency_id', 'deleted',
                                      'invoiceinstallment_id', 'invoicepayments_id', 'issuedate',
                                      'paymentdate', 'paymentstype_id', 'paymentsreference']],
               ['arpaymentstype', ['cashflow', 'code', 'deleted', 'description', 'signal',  'transactiontype_id',  'updatebalance']],
               ['arinvoicebra', None],
               ['arinvoiceorigin', None]
              ]
...
def nfe_parser(invoicebra, invoices_df):
    '''
    ...
    xml_parser_df = invoicebra.apply(lambda row: parse_nfe_invoicebra(row), axis=1, result_type='expand')

    xml_parser_df.columns=['emitente',
                           'serie',
                           'numero',
                           'emissao',
                           'cliente',
    ...
    invoices_df = invoices_df.merge(xml_parser_df,
                                      ...
                                      right_on=['orgid', 'invoiceid'],
                                      how='left')
    
    return invoices_df


def get_returns(invoiceorigin, invoices_df):
    '''
...
def default_techfin_data_parser(connector, max_workers=1):
    ...
    downloaded_tables = getData(stag, tables, connector, max_workers)
    ...
    ar_invoices_df = nfe_parser(downloaded_tables['arinvoicebra'], ar_invoices_df)
    
    # Identifica as devoluções
    ar_invoices_df = get_returns(downloaded_tables['arinvoiceorigin'], ar_invoices_df)
