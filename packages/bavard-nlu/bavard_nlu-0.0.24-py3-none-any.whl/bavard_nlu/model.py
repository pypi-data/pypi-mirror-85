...
from bavard_nlu.data_preprocessing.nlu_data import NLUDataPreprocessor, NLUDataUtils
from bavard_nlu.utils import aggregate_dicts, leave_one_out, HuggingFaceTokenizerSerializer
from bavard_nlu.auto_setup import AutoSetup
from bavard_nlu import constants
...
class NLUModel(WebService):
    ...
    serializer = Serializer(
        HuggingFaceTokenizerSerializer(DistilBertTokenizerFast)
    )
    ...
    def to_dir(self, path: str) -> None:
        ...
        if not os.path.exists(path):
            os.makedirs(path)
        self.serializer.serialize(self, self._resolve_serialization_path(path))
    ...
    def from_dir(path: str, delete: bool = False) -> "NLUModel":
        ...
        """
        model = NLUModel.serializer.deserialize(NLUModel._resolve_serialization_path(path))
        if delete:
            shutil.rmtree(path)
        return model
    
    @staticmethod
    def _resolve_serialization_path(path: str) -> str:
        return os.path.join(path, "model.tar")
    ...
    def evaluate(
        ...
        *,
        ...
        if test_ratio is not None:
            ...
            eval_fn = lambda: self._evaluate_kfold_cv(nlu_data, auto, nfolds)
        else:
    ...
    def _evaluate_kfold_cv(self, nlu_data: dict, auto: bool, nfolds: int) -> tuple:
