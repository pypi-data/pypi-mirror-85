...
class HuggingFaceTokenizerSerializer(TypeSerializer):
    type_name = "tokenizer"
    ext = None

    def __init__(self, TokenizerClass: t.Type[PreTrainedTokenizerBase]) -> None:
        """
        Parameters
        ----------
        TokenizerClass : class inheriting `PreTrainedTokenizerBase`.
            During deserialization, tokenizer states will be loaded into
            an instance of this class.
        """
        self.TokenizerClass = TokenizerClass

    def serialize(self, obj: PreTrainedTokenizerBase, path: str) -> None:
        obj.save_pretrained(path)

    def deserialize(self, path: str) -> object:
        return self.TokenizerClass.from_pretrained(path)
