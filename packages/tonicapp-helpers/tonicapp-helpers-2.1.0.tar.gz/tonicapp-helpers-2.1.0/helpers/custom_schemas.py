...
class BaseSchema(AutoSchema):
    def __init__(self, **kwargs):
        ...
        self.request_serializers = kwargs.pop("request_serializers", None)
    ...
    def get_components(self, path, method):
        ...
        elif self.request_serializers:
            for version in self.request_serializers.keys():
                if version in path:
                    serializer = self.request_serializers[version]()
                    break
            content = self.map_serializer(serializer)
        else:
