...
class DiskHandler(tornado.web.StaticFileHandler, BaseHandler):
    ...
    async def get(self, name, include_body=True):
        ...
        if self.args.q:
            ...
            self.finish(info)
        elif self.request.path.startswith('/file/') or self.args.f == 'download' or re.match('wget|curl', self.ua.lower()):
            self.set_header('Content-Type', 'application/octet-stream')
            zh = re.compile('[\u4e00-\u9fa5]+')
            ...
            elif re.match('(py|sh|h|c|cpp|js|css|html|java|go|ini|conf|vue|txt|cfg)$', suffix):
                code = {
                    'py': 'python',
                    'sh': 'bash',
                    'h': 'c',
            ...
            elif re.match('(jpeg|jpg|png|gif|bmp|svg|tif|webp|ai|ico|exif)$', suffix):
                await self.send(name, include_body)
            else:
    ...
    async def upload(self, path):
        if not 'upload' in self.app.roles:
            raise tornado.web.HTTPError(403)
    ...
    async def delete(self, name):
        if not 'delete' in self.app.roles:
            raise tornado.web.HTTPError(403)
