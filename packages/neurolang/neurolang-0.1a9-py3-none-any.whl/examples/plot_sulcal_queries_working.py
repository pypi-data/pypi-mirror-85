...
with nl.environment as e:
    ...
    e.left_parietal_lobe_sulcus[e.region] = (
        ...
        & e.anterior_of(e.region, e.destrieux.s["L S_parieto-occipital"]
        )
    )
    e.left_temporal_lobe_sulcus[e.region] = (
        ...
        & e.anatomical_anterior_of(e.region, e.destrieux.s["L S_parieto-occipital"]
...
# %%
# Example 3: Query the Superior Rostral Sulcus
# ............................................
# this query and all defined will not be in the program
# after the `with` context finishes. But the results
# remain.
# In this query we express that the superior rostral sulcus (SRS)
# as a sulcus which:
#
# * belongs to the left frontal lobe
# * its principal direction is along the anterior-posterior
...
with nl.scope as e:
    e.named_sulcus["L superior rostral sulcus", e.region] = (
        e.left_frontal_lobe_sulcus(e.region)
        & e.principal_direction(e.region, "AP")
        & ~nl.exists(
            e.other_region,
            e.left_frontal_lobe_sulcus(e.other_region)
            & (e.region != e.other_region)
            & e.right_of(e.other_region, e.region),
        )
    )
...
# %%
# Example 3: Query the Intermediate primus of Jensen
# ............................................
# this query and all defined will not be in the program
# after the `with` context finishes. But the results
# remain.
# In this query we express that the intermediate primus of Jensen (IPOJ)
# as a sulcus which:
#
# * belongs to the left frontal lobe
# * its principal direction is along the anterior-posterior
#   axis.
# * no other sulcus satisfying the same conditions is
#   superior to the IPOJ.

with nl.scope as e:
    e.named_sulcus["L interm prim of Jensen", e.region] = (
        e.left_parietal_lobe_sulcus(e.region)
        & e.principal_direction(e.region, "SI")
        & ~nl.exists(
            e.other_region,
...
with nl.scope as e:
    e.named_sulcus["L inferior temporal sulcus", e.region] = (
        ...
        & ~nl.exists(
            e.other_region,
            e.left_temporal_lobe_sulcus(e.other_region)
            & (e.region != e.other_region)
            & e.left_of(e.other_region, e.region),
        )
    )
