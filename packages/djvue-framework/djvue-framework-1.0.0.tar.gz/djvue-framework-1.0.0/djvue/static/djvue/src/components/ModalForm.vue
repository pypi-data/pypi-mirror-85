    ...
    <div>
        <slot name="modal-open-button" v-bind:buttons="{Open:Open,isDetail,isCreate,isUpdate,isDelete}">
            <b-button @click="Open"
                      v-bind="button_props"
            >
              <span v-if="buttonIcon" v-html="buttonIcon"></span>{{buttonLabel}}
            </b-button>
        </slot>
...
export default class ModalForm extends BaseComponent {
    ...
    get buttonIcon(){
      ...
      if (this.isCreate) {
            return '<i class="mr-1 fa fa-plus"></i>'
        } else if (this.isUpdate) {
            return '<i class="mr-1 fa fa-pen"></i>'
        } else if (this.isDelete) {
            return '<i class="mr-1 fa fa-trash"></i>'
        } else if (this.isDetail) {
            return '<i class="mr-1 fa fa-eye"></i>'
        }
        return false
    }
