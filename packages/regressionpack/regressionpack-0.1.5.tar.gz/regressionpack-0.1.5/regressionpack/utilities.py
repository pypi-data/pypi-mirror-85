...
def MatInv(A:np.ndarray, axis:Tuple[int,int]=(-2,-1), rcond:float = 1e-15):
    """
...
class Scaler:
    ...
    _sigX:  np.ndarray
    _Axis:  int
    _noSpread: np.ndarray

    def __init__(self, x:np.ndarray, axis:int = -1):
    ...
    def Scale(self, x:np.ndarray = None) -> np.ndarray:
        """
