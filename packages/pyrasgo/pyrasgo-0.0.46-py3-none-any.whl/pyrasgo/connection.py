...
from pyrasgo.session import Session, generate_headers


class Connection(object, metaclass=Session):
    """
    ...
    def _url(self, resource, api_version=None):
        if '/' == resource[0]:
            resource = resource[1:]
        return f"https://{self._environment.value}/{'' if api_version is None else f'v{api_version}/'}{resource}"
