...
class DBReader:
	...
	def read_ts_index_daily(
			self, start, end, fields='*', code=None, shift=0, forward=0, dbpath=None, con=None):
		"""
	...
	def read_ts_moneyflow_hsgt(
			self, start, end, fields='*', shift=0, forward=0, dbpath=None, con=None):
		"""
		:param start: 开始日期，必选，支持int(20180808), datetime('2018-08-08 00:00:00'), str('20180808')
		:param end: 结束日期 ，必选，支持同start
		:param fields: 要从库中取出的列名,默认取所有，支持str和list[str]，例如'code,high', ['code','high']
	...
	def read_cal_market_1min_data(
		...
		connect = self._get_connect(conn=con, dbpath=dbpath, dbname=dbname)
		col = self.get_tableinfo_cols(
			tablename='cal_market_1min_data',
			con=connect
		)
		start_date = self.check_time(time=start)
		end_date = self.check_time(time=end)
	...
	def query_stock_day(
			self, start, end, tablename, fields='*', code=None, shift=0, forward=0, newdrop=0,
	...
	def query_stock_min(
			self, start, end, tablename, fields='*', code=None, shift=0, forward=0,
	...
	def query_index_day(
			self, start, end, tablename, fields='*', code=None, shift=0, forward=0, dbpath=None, con=None, dbname=None):
		"""
	...
	def query_market_min(
			self, start, end, tablename, fields='*', shift=0, forward=0, time_start=None,
	...
	def query_market_day(
			self, start, end, tablename, fields='*', shift=0, forward=0,
	...
	def query_stock_limitday(
			self, date, code, tablename='ts_day_data', fields='*',
