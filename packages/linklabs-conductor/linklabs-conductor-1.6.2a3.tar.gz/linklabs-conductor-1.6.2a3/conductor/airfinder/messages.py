...
class DownlinkMessageSpec:
    ...
    def _compile_var_data_struct(self, msg_def, msg_struct, msg_defaults, **kwargs):
        data_lens = []
        for key in msg_def:
            if 'data' in key:
                data = kwargs.get(key)
    ...
    def build_ctrl_message(self, ctrl_cmd, **kwargs):
        ...
        if ctrl_def and ('data' in ctrl_def or 'data1' in ctrl_def):
            ctrl_struct = self._compile_var_data_struct(ctrl_def, ctrl_struct, ctrl_defaults, **kwargs)
