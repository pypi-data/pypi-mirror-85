...
class DownlinkMessageSpec:
    ...
    def build_ctrl_message(self, ctrl_cmd, **kwargs):
        ...
        # Calculate Variable Length Elements when nessisary.
        if ctrl_def and 'data' in ctrl_def:
            if 'data' in kwargs:
                data = kwargs.get('data')
                if data:
                    # Write size of data.
                    ctrl_struct = ctrl_struct.format(len(data))
                else:
                    # Remove Data Element.
                    ctrl_struct = ctrl_struct.replace('{}s', '')
                    ctrl_def.remove("data")
                LOG.debug("updated struct: {}".format(ctrl_struct))
            else:
                raise MissingDownlinkArgument('data')
