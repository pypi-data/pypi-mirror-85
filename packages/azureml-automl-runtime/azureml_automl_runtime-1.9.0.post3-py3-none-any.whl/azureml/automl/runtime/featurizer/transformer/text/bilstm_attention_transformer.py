...
from azureml.automl.core.shared.exceptions import FitException, TransformException, DataShapeException
    ...
    def split_train_valid_set(x_train, y_train, split_ratio):
        try:
            ...
            if "The least populated class in y has only 1" in str(e):
                raise DataShapeException.from_exception(
                    e,
                    msg="BiLSTM train-valid split encountered class with only 1 row",
                    target="BiLSTM - train_valid_split",
                    reference_code=ReferenceCodes._BILSTM_SPLIT_TRAIN_VALID_ONLY_ONE_ROW,
                    has_pii=False)
