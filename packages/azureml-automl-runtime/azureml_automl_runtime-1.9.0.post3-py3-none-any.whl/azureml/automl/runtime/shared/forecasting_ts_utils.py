...
from azureml.automl.core.shared.exceptions import (
    InvalidDataTypeException, AllLabelsMissingException, EmptyDataException, InsufficientDataException)
...
def last_n_periods_split(tsdf: 'TimeSeriesDataFrame',
    ...
    try:
        ...
        raise InvalidDataTypeException('Test size should be numeric.', has_pii=False)

    if not isinstance(tsdf, TimeSeriesDataFrame):
        raise InvalidDataTypeException('Input must be a data frame.', has_pii=False)

    if test_size <= 0:
        raise EmptyDataException("Expected 'test_size' > 0", has_pii=False)
