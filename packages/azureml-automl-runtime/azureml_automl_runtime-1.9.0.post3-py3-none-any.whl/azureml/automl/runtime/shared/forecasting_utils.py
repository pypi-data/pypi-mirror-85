...
from azureml.automl.core.shared.exceptions import InvalidDataTypeException
...
def get_period_offsets_from_dates(reference_index, time_index, freq,
    ...
    if freq is None:
        ...
        raise DataFrameFrequencyException(error_message, has_pii=False)
    # Check if frequency is valid.
    try:
        to_offset(freq)
    except ValueError as e:
        raise DataFrameFrequencyException.from_exception(
            e, has_pii=True).with_generic_msg('Incorrect frequency string.')
    ...
    if any(ref not in date_grid for ref in reference_index):
        ...
        if misalignment_action == 'warn':
            ...
            raise DataFrameFrequencyException(
                msg.format(date_grid)).with_generic_msg(msg.format('[Masked]'))
...
def subtract_list_from_list(minuend, subtrahend):
    ...
    if not isinstance(minuend, list):
        raise InvalidDataTypeException('Minuend must be of type {0}'.format(list), has_pii=False)
    if not isinstance(subtrahend, list):
        raise InvalidDataTypeException('subtrahend must be of type {0}'.format(list), has_pii=False)
...
def invert_dict_of_lists(dictionary):
    ...
    if not isinstance(dictionary, dict):
        raise InvalidDataTypeException('Input must be a dictionary!', has_pii=False)
