...
from mimosa.models import FirestoreTuple
...
def get_company(site_key: str, company_id: str) -> dict:
    """Download a site key company document and return the data as a dictionary"""
    if site_key == "":
        raise ValueError("Site key was an empty string.")
    if company_id == "":
        raise ValueError("company_id was an empty string.")

    db = firestore.client()
    doc = db.document(f"siteKeys/{site_key}/siteKeyCompanies/{company_id}").get()
    if doc.exists is False:
        raise NotFound(f"Company {company_id} in {site_key} does not exist.")
    return doc.to_dict()
...
def get_sk_user_permissions(*, site_key: str, uid: str) -> dict:
    """Download and return a site key user permissions document as a dictionary"""
    """Download and return a site key user document"""
    if site_key == "":
        raise ValueError("Site key was an empty string.")
    if uid == "":
    ...
    db = firestore.client()
    doc = db.document(
        f"siteKeys/{site_key}/siteKeyUsers/{uid}/privateColl/privateDoc"
    ).get()
    if doc.exists is False:
        raise NotFound(f"Permissions document for {uid} was not found.")
    return doc.to_dict()
...
def query_all_site_key_companies(site_key: str) -> List[FirestoreTuple]:
    """
    ...
    result_list = [FirestoreTuple(doc.id, doc.to_dict()) for doc in docs]
    if len(result_list) == 0:
        raise NotFound("No site key companies were found. Please create one first.")
    return result_list


def query_all_sk_users(site_key: str) -> List[FirestoreTuple]:
    ...
    return [FirestoreTuple(doc.id, doc.to_dict()) for doc in docs]
