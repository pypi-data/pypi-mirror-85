...
def get_depth_file(index_ref, ref_seq, reads_r1, reads_r2, reads_unpaired, num_threads):
    ...
    if reads_unpaired == '':
        cmd_bowtie2_mapping = 'bowtie2 -x %s/%s -1 %s -2 %s -S %s -p %s -f' % (ref_seq_file_path, ref_seq_file_basename, reads_r1, reads_r2, sam_file, num_threads)
    else:
        cmd_bowtie2_mapping = 'bowtie2 -x %s/%s -1 %s -2 %s -U %s -S %s -p %s -f' % (ref_seq_file_path, ref_seq_file_basename, reads_r1, reads_r2, reads_unpaired, sam_file, num_threads)
...
def link_Marker_MAG(args, config_dict):
    ...
    for unmapped_read in unmapped_paired_reads_to_ctg_dict:
        ...
        print()
        print('unmapped_read_base_r1_matched')
        print(unmapped_read_base_r1_matched)
        print()
        print('unmapped_read_base_r2_matched')
        print(unmapped_read_base_r2_matched)
        print()
...
~/PycharmProjects/MarkerMAG/MarkerMAG/link_Marker_MAG.py -p Test -r1 combined_5x_R1.fasta -r2 combined_5x_R2.fasta -m combined_16S.ffn -mag selected_genomes_renamed_no_plasmid -x fna -t 4 -tmp -force -test_mode
