...
metadata = {'creator': {'name': 'Unknown',
                        'affiliation': 'Unknown',
                        'time': 000000},
            'data source': {'origin': {'owner': 'Unknown',
...
class Foreperson:
    ...
    def ready(self, directory):
        directory_path = directory.format(self.metadata['data source']['experiment']['instrument'], self.year, self.visit_id)
        if path.isdir(directory_path):
            self.directory_path = directory_path
        else:
            raise FileNotFoundError("The experiment directory cannot be found.")

def reduce(run_numbers, yaml_file, directory='/dls/{}/data/{}/{}/', metadata=metadata):
    """
    ...
    the_boss.metadata['reduction']['input files']['datafile'] = [directory + str(r) for r in run_numbers]
    keys = recipe.keys()
    ...
    the_boss.metadata['reduction']['data state']['rebinned'] = the_boss.q_space

    the_boss.metadata['data source']['experiment']['measurement']['q range'] = [refl.q.min(), refl.q.max()]
    ...
    if the_boss.output_columns == 3:
        ...
        np.savetxt(the_boss.directory_path + '/processing/XRR_' + run_numbers[0] + '.dat', data, header='{}'.format(yaml.dumps(the_boss.metadata)))
