...
class Foreperson:
    ...
    def setup(self, recipe):
        ...
        if 'background' in keys:
            ...
            self.metadata['data source']['origin']['experiment date'] = str(datetime.datetime.now().date())
        ...
        if 'setup' in keys:
            ...
            if 'theta_axis' in recipe['setup'].keys():
                self.experiment.measurement.theta_axis_name = recipe['setup']['theta_axis']
            if 'q axis' in recipe['setup'].keys():
                self.experiment.measurement.q_axis_name = recipe['setup']['q axis']
            if 'transpose' in recipe['setup'].keys():
                self.experiment.measurement.transpose = recipe['setup']['transpose']
                if self.experiment.measurement.transpose:
                    self.experiment.measurement.qz_dimension = 0
                    self.experiment.measurement.qxy_dimension = 1
            if 'pixel_max' in recipe['setup'].keys():
                self.experiment.measurement.pixel_max = recipe['setup']['pixel_max']
            if 'hot_pixel_max' in recipe['setup'].keys():
                self.experiment.measurement.hot_pixel_max = recipe['setup']['hot_pixel_max']
        else:
