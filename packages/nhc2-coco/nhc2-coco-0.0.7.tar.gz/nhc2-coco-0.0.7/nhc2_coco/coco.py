...
class CoCo:
    def __init__(self, address, username, password, port=8883, ca_path=None, switches_as_lights=[]):
        if ca_path is None:
            ...
            if topic == (self._profile_creation_id + MQTT_TOPIC_SUFFIX_RSP):
                if response['Method'] == 'devices.list':
                    ...
                    lights = [x for x in devices if
                              (x['Model'] == 'light')
                              or (
                                      (x['Model'] == 'switched-generic')
                                      and (
                                              (self._switches_as_lights.count(x['Name']) > 0)
                                              or
                                              (self._switches_as_lights.count(x['Uuid']) > 0)
                                      )
                              )
                              ]
