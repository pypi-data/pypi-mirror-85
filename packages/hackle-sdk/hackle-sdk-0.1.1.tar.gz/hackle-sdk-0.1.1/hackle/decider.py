...
class Decider(object):
    ...
    def decide(self, experiment, user_id):
        ...
        if isinstance(experiment, entities.RunningExperiment):
            return self.__decide(experiment, user_id)

    def __decide(self, experiment, user_id):
        variation = experiment.userOverrides.get(user_id)
        if variation:
            return Decision(None, variation.key, enums.DecisionType.FORCED)

        slot = self.bucketer.bucketing(experiment.bucket, user_id)
        if not slot:
            return None
