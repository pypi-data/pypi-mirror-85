...
class Config(object):
    ...
    def predict_triples(self, triples: Tuple):
        if self.importName != None:
            self.restore_tensorflow()
        res = self.test_step(np.array([triple[0] for triple in triples]), np.array([triple[1] for triple in triples]), np.array([triple[2] for triple in triples]))
        return res
